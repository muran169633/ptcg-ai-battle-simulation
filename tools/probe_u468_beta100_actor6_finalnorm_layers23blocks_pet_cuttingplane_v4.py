#!/usr/bin/env python3
"""Thin frozen extension of combined v1 with complete transformer layers 2 and 3.

The mutable vector is the frozen actor6 prefix, final LayerNorm, and all
twenty-four parameters of transformer layers 2 and 3: thirty-two FP32 tensors /
462,593 scalars.  Only the actor6 prefix is projected out of the frozen P/E/T span.
...
TOOLS = ROOT / "tools"
SCRIPT = TOOLS / "probe_u468_beta100_actor6_finalnorm_layers23blocks_pet_cuttingplane_v4.py"
EXPECTED_PYTHON = Path("/home/xxc/miniconda3/envs/my_project_env/bin/python")
SCHEMA = "ptcg-u468-beta100-actor6-finalnorm-layers23blocks-pet-cuttingplane-v4"
...
LAYERS23BLOCKS_ELEMENTS = 396544
SHARED_TAIL_ELEMENTS = FINALNORM_ELEMENTS + LAYERS23BLOCKS_ELEMENTS
...
EXPECTED_BETA_VARIABLE32_SHA256 = (
    "2c492dd08accac40bc9c23a30492688e2188e5304aea2f2974b1c1223e5b843f"
)
EXPECTED_BETA_OTHER48_SHA256 = (
    "858d333733e5323fa16b64b2904998013631b64c4f1977ad5c79905359038ec1"
)
...
NORM_NAMES = FINAL_NORM_NAMES + LAYERS23_BLOCK_NAMES
...
def validate_runtime() -> None:
    ...
    if len(VARIABLE_NAMES) != 32 or COMBINED_ELEMENTS != 462593:
        raise RuntimeError("v4 variable manifest drift")
    if HARD_COMBINED_L2 != BASE.HARD_COMBINED_L2:
        raise RuntimeError("v4 attempted to relax the combined radius")


def endpoint_audit_v4(vector: np.ndarray, q: np.ndarray) -> dict[str, Any]:
    value = np.asarray(vector, dtype=np.float64)
    if value.shape != (COMBINED_ELEMENTS,):
        raise RuntimeError(f"v4 endpoint shape drift: {value.shape}")
    if q.shape != (COMBINED_ELEMENTS, 3):
        raise RuntimeError(f"v4 embedded-Q shape drift: {q.shape}")
    components = q.T @ value
    actor = value[:ACTOR_ELEMENTS]
    finalnorm = value[ACTOR_ELEMENTS : ACTOR_ELEMENTS + FINALNORM_ELEMENTS]
    layers23blocks = value[ACTOR_ELEMENTS + FINALNORM_ELEMENTS :]
    combined_l2 = float(np.linalg.norm(value))
    actor_l2 = float(np.linalg.norm(actor))
    finalnorm_l2 = float(np.linalg.norm(finalnorm))
    layers23blocks_l2 = float(np.linalg.norm(layers23blocks))
    ...
    return {
        ...
        "layers23blocks_l2": layers23blocks_l2,
        "all_shared_tail_l2": float(math.hypot(finalnorm_l2, layers23blocks_l2)),
...
def build_combined_geometry_v4(basis: Mapping[str, Any]) -> dict[str, Any]:
    q_actor = np.ascontiguousarray(basis["q"].numpy(), dtype=np.float64)
    if q_actor.shape != (ACTOR_ELEMENTS, 3):
        raise RuntimeError("v4 actor PET Q shape drift")
    ...
    if residual > 1.0e-12 or bool(np.any(q[ACTOR_ELEMENTS:] != 0.0)):
        raise RuntimeError("v4 embedded PET geometry drift")
    ...
    checks = {
        "variable_tensor_count_exact_32": len(variable) == 32,
        "combined_elements_exact_462593": (
        ...
        "other_tensor_count_exact_48": len(other) == 48,
        "variable32_sha_exact": (
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v4 beta100 state geometry drift: {checks}")
    return {
        "q": q,
        "q_actor": q_actor,
        "report": {
            ...
            "layers23blocks_elements": LAYERS23BLOCKS_ELEMENTS,
            ...
            "pet_span": "frozen actor6 P/E/T with 396800 zero shared-tail rows",
            "beta_variable32_sha256": EXPECTED_BETA_VARIABLE32_SHA256,
            "beta_other48_sha256": EXPECTED_BETA_OTHER48_SHA256,
            "checks": checks,
        },
    }
...
def _binding_values() -> dict[str, Any]:
    return {
        ...
        "COMBINED_ELEMENTS": COMBINED_ELEMENTS,
        "EXPECTED_BETA_VARIABLE8_SHA256": EXPECTED_BETA_VARIABLE32_SHA256,
        "EXPECTED_BETA_OTHER72_SHA256": EXPECTED_BETA_OTHER48_SHA256,
        "build_combined_geometry": build_combined_geometry_v4,
        "endpoint_audit": endpoint_audit_v4,
    }
...
def static_audit() -> dict[str, Any]:
    ...
    checks = {
        ...
        "scope_exact_32_tensors_462593": (
            len(VARIABLE_NAMES) == 32 and COMBINED_ELEMENTS == 462593
        ),
        "shared_partition_exact_256_plus_396544": (
            FINALNORM_ELEMENTS == 256
            and LAYERS23BLOCKS_ELEMENTS == 396544
            and SHARED_TAIL_ELEMENTS == 396800
            and NORM_ELEMENTS == SHARED_TAIL_ELEMENTS
        ),
        "one_hard_combined_radius_unchanged": HARD_COMBINED_L2 == 1.0e-3,
        "solver_self_test_under_v4_bindings": solver_test["pass"],
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v4 static audit failed: {checks}")
    return {
        "status": "static_layers23blocks_v4_audit_passed",
        "checks": checks,
...
def _normalize_result_labels(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(scope, dict):
        scope["other_state_tensors_exact"] = 48
        scope["finalnorm_elements"] = FINALNORM_ELEMENTS
        scope["layers23blocks_elements"] = LAYERS23BLOCKS_ELEMENTS
    ...
    if isinstance(integrity, dict):
        if "variable8_tensor_restored_exact" in integrity:
            integrity["variable32_tensor_restored_exact"] = integrity.pop(
                "variable8_tensor_restored_exact"
            )
        if "other72_sha_after" in integrity:
            integrity["other48_sha_after"] = integrity.pop("other72_sha_after")
        if "other72_restored_exact" in integrity:
            integrity["other48_restored_exact"] = integrity.pop(
...
def run_probe(
    ...
    def enriched_consumer(payload_source: Mapping[str, Any]) -> None:
        if candidate_consumer is None:
            raise RuntimeError("internal v4 consumer contract drift")
        ...
        if int(vector.numel()) != COMBINED_ELEMENTS:
            raise RuntimeError("v4 consumer vector shape drift")
        final = vector[ACTOR_ELEMENTS : ACTOR_ELEMENTS + FINALNORM_ELEMENTS]
        layers23blocks = vector[ACTOR_ELEMENTS + FINALNORM_ELEMENTS :]
        payload["actual_finalnorm_delta_l2"] = float(final.double().norm())
        payload["actual_layers23blocks_delta_l2"] = float(
            layers23blocks.double().norm()
        )
        payload["actual_shared_tail_delta_l2"] = float(
            vector[ACTOR_ELEMENTS:].double().norm()
        )
        payload["v4_variable_names"] = list(VARIABLE_NAMES)
...
def cache_audit() -> dict[str, Any]:
    ...
    result["status"] = "layers23blocks_v4_cache_audit_passed_zero_write_train_only"
    result["self"] = read_regular_bytes(SCRIPT, None, "layers23blocks v4 self")[1]
...
def main() -> None:
    ...
    if args.mode == "static-audit":
        result: dict[str, Any] = {
            ...
            "self": read_regular_bytes(SCRIPT, None, "layers23blocks v4 self")[1],
