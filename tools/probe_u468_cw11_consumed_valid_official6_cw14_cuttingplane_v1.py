#!/usr/bin/env python3
"""CW14 anchored-total consumed-valid official-B256 cutting-plane probe.
...
SCRIPT = TOOLS / "probe_u468_cw11_consumed_valid_official6_cw14_cuttingplane_v1.py"
SCHEMA = "ptcg-cw14-consumed-valid-official-b256-anchored-total-cuttingplane-v1"
SEED = 202608207
FROZEN_MODE = 0o555
...
def static_source_audit(source: bytes) -> dict[str, Any]:
    ...
    gradients_segment = segments.get("official_context_gradients", "")
    ...
    checks = {
        ...
        == CW13_THRESHOLD_FUNCTION_SHAS,
        "every_QP_linearizes_all_cuts_at_exact_CW11_anchor": all(
            token in gradients_segment
            for token in (
                "linearization_anchor_model_sha256",
                "live_model_exact_CW11",
                "official_snapshots_are_frozen_references",
                "cut_gradient_records",
                "linearization_record_sha256",
                "observed_anchor_margin",
                "threshold",
                "rhs",
            )
        )
        ...
        "anchored_minimum_total_replaces_cumulative_minimum_step": all(
            token in outer_segment
            for token in (
                "anchored_solution",
                "proposal_additional = np.asarray",
                "minimum total additional from exact CW11 anchor",
            )
        )
        ...
        "prior_active_linearization_records_must_repeat_exact": all(
            token in outer_segment
            for token in (
                "anchor_linearization_registry",
                "gradient_float64_le_sha256",
                "linearization_record_sha256",
                "prior_active_linearization_records_exact",
            )
        ),
        "nested_minimum_total_geometry_is_hard_gated": all(
            token in outer_segment
            for token in (
                "active_cut_set_nested",
        ...
        "anchor_restore_gradient_QP_order_exact": min(outer_positions.values())
        >= 0
        and outer_positions["restore_cw11_anchor_for_total_qp"]
        < outer_positions["official_context_gradients"]
        ...
        "official_matmul_precision_high_scoped_and_restored": all(
            token in segments.get("run_cw14_consumer", "")
            for token in (
...
def frozen_modules() -> dict[str, ModuleType]:
    return {
        ...
        "cw10": import_frozen(
            CW10, MODULE_SHAS[CW10], EXPECTED_INPUT_MODES[CW10], "cw14_frozen_cw10"
        ),
        "cw9": import_frozen(
            CW9, MODULE_SHAS[CW9], EXPECTED_INPUT_MODES[CW9], "cw14_frozen_cw9"
        ),
        ...
        "geometry": import_frozen(
            ...
            "cw14_frozen_geometry",
        ),
        "ram": import_frozen(
            RAM, MODULE_SHAS[RAM], EXPECTED_INPUT_MODES[RAM], "cw14_frozen_ram"
        ),
        "legacy": import_frozen(
            FORENSIC_ANALYZER,
            MODULE_SHAS[FORENSIC_ANALYZER],
            EXPECTED_INPUT_MODES[FORENSIC_ANALYZER],
            "cw14_frozen_forensic_analyzer",
        ),
    }
...
def official_context_gradients(
    ...
    *,
    linearization_anchor_model_sha256: str,
    ...
    anchor_checks = {
        "requested_anchor_exact_CW11": linearization_anchor_model_sha256
        == CW11_MODEL_SHA256,
        "live_model_exact_CW11": helper.model_state_sha256(model.state_dict())
        == CW11_MODEL_SHA256,
        "model_eval_exact": model.training is False,
        "legacy_snapshot_is_frozen_reference": legacy["candidate_snapshot"]
        is legacy["reference_snapshot"],
        "official_snapshots_are_frozen_references": all(
    ...
    if not all(anchor_checks.values()):
        raise RuntimeError(f"CW14 gradient anchor drift: {anchor_checks}")
    ...
    for context_hash in sorted(groups):
        ...
        for index, cut in enumerate(cuts):
            ...
            gradient_sha_by_key[key] = hashlib.sha256(
                np.ascontiguousarray(flat.astype("<f8")).tobytes()
            ).hexdigest()
    ...
    for value in ordered:
        key = cut_identity(value)
        core = {
            "identity": list(key),
            "gradient_float64_le_sha256": gradient_sha_by_key[key],
            "observed_anchor_margin": float(observed_by_key[key]),
            "threshold": float(value["threshold"]),
            "rhs": float(value["threshold"])
            - float(observed_by_key[key]),
        }
        cut_gradient_records.append(
            {**core, "linearization_record_sha256": canonical_sha(core)}
        )
    return matrix, rhs, {
        ...
        "linearization_anchor": "frozen_CW11_4317",
        "linearization_anchor_model_sha256": CW11_MODEL_SHA256,
        "anchor_checks": anchor_checks,
        "matrix_and_rhs_finite": bool(
            np.isfinite(matrix).all() and np.isfinite(rhs).all()
        ),
        "cut_gradient_records": cut_gradient_records,
        "cut_linearization_ledger_sha256": canonical_sha(cut_gradient_records),
...
def restore_cw11_anchor_for_total_qp(
    ...
    for view in VIEW_ORDER:
        for batch in streams[view]:
            if batch["reference_snapshot"] is None:
                raise RuntimeError("CW14 anchor restore lacks frozen reference snapshot")
            batch["candidate_snapshot"] = batch["reference_snapshot"]
            official_batch_count += 1
    if legacy["reference_snapshot"] is None:
        raise RuntimeError("CW14 anchor restore lacks legacy reference snapshot")
    ...
    if not all(checks.values()):
        raise RuntimeError(f"CW14 exact CW11 anchor restore failed: {checks}")
    return {
        "anchor_model": "frozen_CW11_4317",
...
def run_outer_cutting_plane(
    ...
    *,
    ...
    if not required_context.issubset(context):
        raise RuntimeError("CW14 incoming CW11 consumer schema drift")
    incoming_checks = {
    ...
    initial_checks = {
        ...
        "CW14_single_variable_caps_unchanged_equal_0p001": STEP_L2_CAP
        == ADDITIONAL_TOTAL_L2_CAP
        == 0.001,
    ...
    if not all(initial_checks.values()):
        raise RuntimeError(f"CW14 initial-state audit failed: {initial_checks}")
    ...
    if (
        ...
        raise RuntimeError("CW14 initial 34+4 active cut ledger drift")
    ...
    if initial_active_gates["violated_count"] != 4:
        raise RuntimeError("CW14 initial fixed repair violation count drift")
    ...
    anchor_linearization_registry: dict[tuple[Any, ...], str] = {}
    prior_qp_active_identities: set[tuple[Any, ...]] = set()
    for iteration in range(1, MAX_OUTER_ITERATIONS + 1):
        anchor_audit = None
        gradient_reuse_audit = None
        try:
            previous_additional = additional.copy()
            current_active_identities = {
                cut_identity(value) for value in active
            }
            ...
            repeated_linearization_checks = []
            new_linearization_records = []
            for record in gradient_audit["cut_gradient_records"]:
                key = tuple(record["identity"])
                observed_sha = str(record["linearization_record_sha256"])
                if key in anchor_linearization_registry:
                    repeated_linearization_checks.append(
                        {
                            "identity": list(key),
                            "gradient_float64_le_sha256": record[
                                "gradient_float64_le_sha256"
                            ],
                            "observed_anchor_margin": record[
                                "observed_anchor_margin"
                            ],
                            "threshold": record["threshold"],
                            "rhs": record["rhs"],
                            "previous_linearization_record_sha256":
                            anchor_linearization_registry[key],
                            "current_linearization_record_sha256": observed_sha,
                            "exact": anchor_linearization_registry[key]
                            == observed_sha,
                        }
                    )
                else:
                    new_linearization_records.append(
                        {
                            "identity": list(key),
                            "gradient_float64_le_sha256": record[
                                "gradient_float64_le_sha256"
                            ...
                            "threshold": record["threshold"],
                            "rhs": record["rhs"],
                            "linearization_record_sha256": observed_sha,
                        }
                    )
            gradient_reuse_audit = {
                "linearization_anchor_exact_CW11": gradient_audit[
                    "linearization_anchor_model_sha256"
                ]
                == CW11_MODEL_SHA256,
                "active_identity_set_exact": {
                    tuple(value["identity"])
                    for value in gradient_audit["cut_gradient_records"]
                }
                == current_active_identities,
                "matrix_and_rhs_finite": gradient_audit[
                    "matrix_and_rhs_finite"
                ],
                "prior_active_linearization_records_exact": all(
                    value["exact"] for value in repeated_linearization_checks
                ),
                "repeated_count": len(repeated_linearization_checks),
                "new_count": len(new_linearization_records),
                "repeated": repeated_linearization_checks,
                "new": new_linearization_records,
            }
            if not all(
                bool(gradient_reuse_audit[key])
                for key in (
                    "linearization_anchor_exact_CW11",
                    "active_identity_set_exact",
                    "matrix_and_rhs_finite",
                    "prior_active_linearization_records_exact",
                )
                ...
                raise RuntimeError(
                    f"CW14 anchored gradient reuse drift: {gradient_reuse_audit}"
                )
            for record in gradient_audit["cut_gradient_records"]:
                anchor_linearization_registry[
                    tuple(record["identity"])
                ] = str(record["linearization_record_sha256"])
            active_cut_set_nested = prior_qp_active_identities.issubset(
                current_active_identities
            )
            prior_qp_active_identities = set(current_active_identities)
            anchored_solution, qp = modules["cutting"].solve_minimum_l2_correction(
                gradients, rhs, np, optimize
            )
            qp = {
                **qp,
                "CW14_objective": "minimum total additional from exact CW11 anchor",
                "linearization_anchor_model_sha256": CW11_MODEL_SHA256,
                "thresholds_unchanged_from_CW13": True,
            }
            if not (
                ...
                raise RuntimeError("CW14 frozen QP/total cap binding drift")
        except Exception as exc:
            ...
            iterations.append(
                {
                    ...
                    "gradient_reuse_audit": gradient_reuse_audit,
                }
            )
            break
        ...
        geometry_checks = {
            "active_cut_set_nested": active_cut_set_nested,
            "minimum_total_norm_monotone_nondecreasing":
        ...
        integrity = {
            ...
            "all_active_linearizations_CW11_anchored_and_repeat_exact": all(
                bool(gradient_reuse_audit[key])
                for key in (
                    "linearization_anchor_exact_CW11",
                    "active_identity_set_exact",
        ...
        if not additions:
            close_reason = (
                "fail_closed_nonfeasible_without_separating_cut"
                if active_gates["violated_count"] == 0
                else "fail_closed_anchored_total_active_violation_without_new_cut"
            )
            break
    ...
    return {
        ...
        "downstream_contract": {
            ...
            "broad_requires_new_CW14_hash_bound_one_shot_runner": True,
        },
    }


def run_cw14_consumer(
    context: Mapping[str, Any],
    modules: Mapping[str, ModuleType],
    *,
    user_candidate_consumer: Any = None,
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError(
                "CW14 local finally failed to restore CW11 or matmul precision"
            )
...
def run_probe(
    ...
    *,
    ...
    def internal_consumer(context: Mapping[str, Any]) -> None:
        if holder:
            raise RuntimeError("CW14 live consumer called more than once")
        holder.update(
            run_cw14_consumer(
                context,
                modules,
    ...
    if not holder:
        raise RuntimeError("CW14 consumer was not reached by exact CW11 reconstruction")
    final_checks = {
    ...
    if not all(final_checks.values()):
        raise RuntimeError(f"CW14 outer reconstruction/finally failed: {final_checks}")
    return {
...
def static_result() -> dict[str, Any]:
    ...
    if stat.S_IMODE(SCRIPT.stat().st_mode) != FROZEN_MODE:
        raise RuntimeError("CW14 script must be frozen mode 0555")
    ...
    return {
        "schema_version": SCHEMA,
        "status": "static_ready_CW14_anchored_total_run_implemented",
        "classification": dict(CLASSIFICATION),
        "contract": {
...
def main() -> None:
    ...
    if args.mode == "static":
        ...
        cw11 = import_frozen(
            ...
            MODULE_SHAS[CW11_PROBE],
            EXPECTED_INPUT_MODES[CW11_PROBE],
            "cw14_main_frozen_cw11",
        )
        source, self_evidence = cw11.read_regular_bytes(
            SCRIPT,
            None,
            "CW14 anchored-total consumed-valid solver",
            expected_mode=FROZEN_MODE,
        )
        source_audit = static_source_audit(source)
        if not source_audit["pass"]:
            raise RuntimeError(f"CW14 source audit failed: {source_audit}")
        primary_source, primary_evidence = cw11.read_regular_bytes(
            PRIMARY,
            MODULE_SHAS[PRIMARY],
            "CW14 frozen primary source",
            expected_mode=FROZEN_MODE,
        )
        primary = import_frozen(
            PRIMARY,
