#!/usr/bin/env python3
"""Audit or execute the frozen U469-U471 raw-U468 PPO continuation."""
...
if stat.S_ISLNK(base_info.st_mode) or not stat.S_ISREG(base_info.st_mode):
    raise RuntimeError("authenticated audit helper must be a regular file")
if hashlib.sha256(BASE.read_bytes()).hexdigest() != BASE_SHA256:
    raise RuntimeError("authenticated audit helper SHA-256 mismatch")
...
SCHEMA = "ptcg-design202608155-u469-u471-u468raw-exactgold12-v1"
SEED = 202608155
...
OUTPUT_ROOT = ROOT / "artifacts/ppo_u468raw_exactgold12_tailfocus_heads3x96_u471_design202608155"
OUTPUT_DIR = OUTPUT_ROOT / "B_gold_league/seed-202608155"
TERMINAL = OUTPUT_DIR / "checkpoints/update-0471.pt"
ATTEMPT = ROOT / ".ptcg-design202608155-u469-u471-attempt.json"
LOG = ROOT / "artifacts/ppo_u468raw_exactgold12_tailfocus_heads3x96_u471_design202608155.log"

DEPENDENCIES = {
    TOOLS / "train_bc_orbit.py": "ee68efc864511d2c88da0429aba8500611e9a2a5a1738573c8073a58cb73a9cc",
    ROOT / "dataset/sample_submission/sample_submission/cg/__init__.py":
        "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
    ROOT / "dataset/sample_submission/sample_submission/cg/sim.py":
...
QUOTAS_BY_POLICY = {
    ...
    "rank17_213tubo": 6,
    "rank18_tuna": 4,
    "rank19_szlachetny": 12,
}

DECK_FILE_SHA256_BY_HASH = {
    ...
    "978ab31c50aa7fdaf11e38ba108cf0eda7fab6ca3597bfa3b45a7f74cd6a9327":
        "7c6399e18e86ec1f19aab25b362d662a27716a660178e3d8ea48d61c2827a1b3",
    "e2e03fe8ef9592b1204c159a7725da3945675fa872b1e18baca550560503a78e":
...
def load_opponents() -> list[dict[str, Any]]:
    if sha256_file(LEAGUE) != LEAGUE_SHA256:
        raise RuntimeError("exact-gold12 league manifest SHA-256 mismatch")
    opponents = json.loads(LEAGUE.read_text(encoding="utf-8"))["opponents"]
    if [item["policy_id"] for item in opponents] != list(QUOTAS_BY_POLICY):
        raise RuntimeError("exact-gold12 opponent order or membership drifted")
    return opponents


def opponent_name(item: dict[str, Any]) -> str:
    return f"{Path(item['checkpoint']).stem}@{Path(item['deck']).stem}"
...
def derive_command() -> list[str]:
    ...
    for item in opponents:
        command.extend(["--extra-opponent", item["checkpoint"], item["deck"]])
    command.extend([
        ...
        "--value-trunk-gradient-scale", "1.0",
    ...
    for item in opponents:
        command.extend([
            "--opponent-base-quota", opponent_name(item),
            str(QUOTAS_BY_POLICY[item["policy_id"]]),
        ])
    return command


def input_bindings(protocol_path: Path) -> dict[Path, str]:
    result = {
        SELF: sha256_file(SELF), protocol_path: sha256_file(protocol_path),
        BASE: BASE_SHA256, TRAINER: TRAINER_SHA256, PARENT: PARENT_SHA256,
        BC: BC_SHA256, REPLAY: REPLAY_SHA256, DECK: DECK_SHA256,
        LEAGUE: LEAGUE_SHA256, **DEPENDENCIES,
    }
    for item in load_opponents():
        result[Path(item["checkpoint"]).resolve()] = item["checkpoint_sha256"]
        result[Path(item["deck"]).resolve()] = DECK_FILE_SHA256_BY_HASH[item["deck_hash"]]
    return {path.resolve(): digest for path, digest in result.items()}


def validate(protocol_path: Path, expected_protocol_sha256: str) -> tuple[dict[str, Any], list[str], dict[str, Any]]:
    ...
    expected = {
        ...
        "updates": [469, 470, 471],
        "games_per_update": 96,
        "fixed_quotas_by_policy": QUOTAS_BY_POLICY,
        "hyperparameters": {
    ...
    if sum(QUOTAS_BY_POLICY.values()) != 96 or any(value % 2 for value in QUOTAS_BY_POLICY.values()):
        raise RuntimeError("quota total or seat-balance contract drifted")
    for target in (OUTPUT_ROOT, OUTPUT_DIR, TERMINAL, ATTEMPT, LOG):
        if target.exists() or target.is_symlink():
            raise FileExistsError(f"refusing existing target: {target}")
    bindings = input_bindings(protocol_path)
    ...
    return protocol, command, {
        ...
        "fixed_quotas_by_policy": QUOTAS_BY_POLICY,
        "fixed_quota_sum": sum(QUOTAS_BY_POLICY.values()),
        "input_files": len(bindings),
        "targets_absent": True,
        "optimizer_steps": 0,
        "rollout_games": 0,
    }
...
def claim_and_run(protocol_path: Path, protocol: dict[str, Any], command: list[str]) -> int:
    marker = {
        "schema_version": SCHEMA,
        "event": "fresh_ppo_attempt_consumed",
        "seed": SEED,
        "protocol_sha256": sha256_file(protocol_path),
        "command_sha256": protocol["command_sha256"],
        "launcher_sha256": sha256_file(SELF),
    }
    marker_fd = os.open(ATTEMPT, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    try:
        os.write(marker_fd, audit.canonical_json_bytes(marker))
        os.fsync(marker_fd)
    finally:
        os.close(marker_fd)
    bindings = input_bindings(protocol_path)
    ...
    try:
        ...
        with os.fdopen(log_fd, "wb", buffering=0) as handle:
            ...
            handle.write(audit.canonical_json_bytes({
                **marker,
                "event": "pre_exec_checks_passed",
                "effective_argv_prefix": effective[:4],
                "input_shared_locks": len(snapshots),
            }))
            environment = os.environ.copy()
            environment.setdefault("CUBLAS_WORKSPACE_CONFIG", ":4096:8")
            completed = subprocess.run(
                effective, cwd=ROOT, stdin=subprocess.DEVNULL,
