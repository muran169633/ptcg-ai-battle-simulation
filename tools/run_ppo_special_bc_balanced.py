#!/usr/bin/env python3
"""Apply a frozen, batch-aligned core-five Gold BC stage after U456.
...
checks while taking zero optimizer steps and writing no checkpoint.
"""
...
OUTPUT_SCHEMA = "ptcg-u456-post-special-bc-balanced-core5-v1"
...
EXPECTED_REPLAY_STEP_BEFORE = 16
EXPECTED_REPLAY_STEP_AFTER = 36
...
def validate_request(args: argparse.Namespace) -> None:
    ...
    if args.expected_parent_update != 456:
        raise ValueError("This protocol requires the frozen U456 parent")
    if args.expected_replay_state_step != EXPECTED_REPLAY_STEP_BEFORE:
        raise ValueError(
            "This protocol must resume the embedded replay AdamW at step 16"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Continue U456's embedded actor BC optimizer on the frozen, "
            "batch-aligned Gold19 core-five archive."
        )
    )
...
def validate_archive_manifest(
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        if manifest.get("seed") != args.seed:
            raise ValueError("Special archive seed does not match --seed")
...
def main() -> None:
    ...
    if int(parent.get("update", -1)) != args.expected_parent_update:
        raise ValueError("Parent checkpoint update does not match frozen U456")
    ...
    if set(replay_steps_before) != {EXPECTED_REPLAY_STEP_BEFORE}:
        raise ValueError("Replay AdamW must start at step 16")
    ...
    if set(replay_steps_after) != {EXPECTED_REPLAY_STEP_AFTER}:
        raise RuntimeError("Replay AdamW did not advance exactly from step 16 to 36")
    ...
    output_checkpoint = output_dir / "special-bc-balanced-core5-0020.pt"
    ...
    if sorted(path.name for path in output_dir.iterdir()) != [
        "special-bc-balanced-core5-0020.pt",
