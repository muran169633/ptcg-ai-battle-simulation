...
# Guard-priority v16: a short head-only continuation from UQ. When BC and v3
# policy gradients conflict, only the BC gradient is projected; the complete
# v3 gradient is retained. A stronger UQ KL anchor limits cumulative drift.
...
case "$mode" in
  full)
    output_dir="${OUTPUT_DIR:-artifacts/ppo_marnie_v16_guard_pcgrad_heads_u458}"
    updates=458
    ...
    ;;
  smoke)
    output_dir="${OUTPUT_DIR:-artifacts/ppo_marnie_v16_guard_pcgrad_smoke_u456}"
    updates=456
    games_per_update=16
    environments=8
    ppo_epochs=2
