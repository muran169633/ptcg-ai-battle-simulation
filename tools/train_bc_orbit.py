...
FEATURE_VERSION = "ptcg-bc-orbit-entity-transformer-v6-priority"
...
TRAINABLE_SCOPES = ("all", "transformer_actor", "actor6")
TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES = (
    "actor_query.weight",
    "actor_key.weight",
...
def state_entities(
    ...
    # The V5 implementation emitted every zone for player 0 before player 1.
    # Once the 80-token cap was reached, that made visibility depend on seat and
    # could drop the opponent board and recent logs behind a large discard pile.
    # Emit semantically critical groups for both players before low-priority
    # history zones so truncation is symmetric and useful at deployment.
    for player_index, player in visible_players:
        entities.append(
            player_summary_entity(
                player,
                relation(player_index, your_index),
                hash_size,
            )
        )
    ...
    if isinstance(stadium_cards, list):
        for position, card in enumerate(stadium_cards):
            entities.append(
                card_entity(card, "public", "stadium", position, hash_size)
            )
    ...
    # Keep summaries, both boards, visible hands, stadium/context, and newest
    # logs ahead of revealed search pools and discard/prize history.
...
def count_training_team_rows(
    ...
    if archive_path.is_dir():
        try:
            ...
            raise RuntimeError(
                "Reading a Parquet BC dataset requires pyarrow"
            ) from error
        members = sorted((archive_path / "train").glob("*.parquet"))
        if not members:
            raise ValueError(
                "policy-team balancing found no train/*.parquet files"
            )
        for member in members:
            parquet_file = pq.ParquetFile(member)
            for batch in parquet_file.iter_batches(
                batch_size=4096,
                columns=("split", "deck_hash", "team_name"),
            ):
                for row in batch.to_pylist():
                    if str(row.get("split", "")) != "train":
                        raise ValueError(
                            f"{member}: non-train row inside train file"
                        )
                    if (
                        allowed_decks
...
def focus_team_weights(
    counts: dict[str, int],
    focus_team_id: str,
    relative_weight: float,
) -> dict[str, float]:
    """Upweight one demonstrator team and keep train-row mean exactly one."""

    # Reuse the strict count validation shared with the existing balancing
    # modes without inheriting their inverse-frequency treatment.
    sqrt_clip2_team_weights(counts)
    if not isinstance(focus_team_id, str) or not focus_team_id:
        raise ValueError("policy focus-team identifier must be non-empty")
    if focus_team_id not in counts:
        raise ValueError(
            f"policy focus team {focus_team_id!r} is absent from train rows"
        )
    if (
        not isinstance(relative_weight, (int, float))
        or isinstance(relative_weight, bool)
        or not math.isfinite(float(relative_weight))
        or float(relative_weight) <= 0.0
    ):
        raise ValueError("policy focus-team relative weight must be positive")

    relative_weight = float(relative_weight)
    total_rows = sum(counts.values())
    unscaled_total = sum(
        count * (relative_weight if team_id == focus_team_id else 1.0)
        for team_id, count in counts.items()
    )
    scale = total_rows / unscaled_total
    weights = {
        team_id: scale * (
            relative_weight if team_id == focus_team_id else 1.0
        )
        for team_id in sorted(counts)
    }
    final_mean = sum(
        counts[team_id] * weights[team_id] for team_id in counts
    ) / total_rows
    if not math.isclose(final_mean, 1.0, rel_tol=0.0, abs_tol=1e-12):
        raise RuntimeError(
            "focus-team weights failed row-mean normalization: "
            f"{final_mean}"
        )
    return weights
...
class ZipDecisionDataset(IterableDataset):
    def __init__(
        ...
        self.select_contexts = set(select_contexts)
                    ...
                    with archive.open(member) as handle:
                        for line in handle:
                            ...
                            if (
                                ...
                                continue
                            if (
                                self.select_contexts
                                and int(row.get("select_context", -1))
                                not in self.select_contexts
                            ):
                                continue
...
class ParquetDecisionDataset(IterableDataset):
    """Stream pre-featurized Parquet row groups with bounded memory."""
    ...
    def __iter__(self) -> Iterator[dict[str, Any]]:
        ...
        member_rng = random.Random(self.shuffle_seed + self.epoch * 10_007)
        if self.split_mode == "all_train":
            members = sorted(self.dataset_dir.rglob("*.parquet"))
        elif self.split_mode == "episode_hash":
            members = sorted(self.dataset_dir.glob("*/*.parquet"))
        else:
            members = sorted(
                (self.dataset_dir / self.split).glob("*.parquet")
            )
        ...
        member_rng.shuffle(work_units)
        active_worker_count = min(worker_count, len(work_units))
        if worker_id >= active_worker_count:
            return
        worker_limit = worker_row_limit(
            self.max_rows,
        ...
        worker_units = work_units[worker_id::worker_count]
        row_groups_by_member: dict[Path, list[int]] = {}
        for member, row_group in worker_units:
            row_groups_by_member.setdefault(member, []).append(row_group)

        def feature_rows() -> Iterator[dict[str, Any]]:
            accepted = 0
            if worker_limit == 0:
                return
            for member, row_groups in row_groups_by_member.items():
                parquet_file = pq.ParquetFile(member)
                for batch in parquet_file.iter_batches(
                    batch_size=512,
...
def collate_decisions(
    ...
    def action_sequence_for_row(row: dict[str, Any]) -> list[int]:
        raw_sequence = row.get("action_sequence")
        if raw_sequence is not None:
            return [int(index) for index in raw_sequence]
        # Orbit v5 predates the explicit sequence field and trained canonical
        # action sets. Reconstruct that historical ascending sequence from the
        # stored target mask instead of rejecting an otherwise valid row.
        return [
            index
            for index, value in enumerate(row.get("targets") or [])
            if float(value) > 0.5
        ]
    ...
    action_sequence_width = max(
        MAX_ACTION_COUNT,
        max(len(action_sequence_for_row(row)) for row in rows),
    )
    ...
    for batch_index, row in enumerate(rows):
        ...
        action_sequence = action_sequence_for_row(row)
        if action_sequence:
            action_sequences[
                batch_index,
                : len(action_sequence),
        ...
        date_weight = (
            daily_weights.get(str(row.get("dataset_date", "")), 1.0)
            if daily_weights
            else 1.0
        )
        sample_weights[batch_index] = row["sample_weight"] * date_weight
...
class TrainConfig:
    ...
    deck_hashes: tuple[str, ...]
    team_names: tuple[str, ...]
    train_select_contexts: tuple[int, ...]
    ...
    policy_team_weights: dict[str, float] | None = None
    policy_team_focus_name: str | None = None
    policy_team_focus_relative_weight: float = 1.25
    init_checkpoint: str | None = None
    init_checkpoint_sha256: str | None = None
    trainable_scope: str = "all"
    expected_trainable_tensors: int | None = None
    expected_trainable_parameters: int | None = None
    daily_equal_weighting: bool = False
    daily_weights: dict[str, float] | None = None
...
def make_loader(
    ...
    common = {
        ...
        "select_contexts": (
            config.train_select_contexts if split == "train" else ()
        ),
    ...
    if data_path.is_dir():
        dataset = ParquetDecisionDataset(
    ...
    return DataLoader(
        ...
        collate_fn=partial(
            ...
            daily_weights=(
                config.daily_weights if split == "train" else None
            ),
        ),
        pin_memory=True,
...
def daily_equal_weights_from_manifest(
    manifest: dict[str, Any],
) -> dict[str, float]:
    """Return train-row weights that give every represented date equal mass."""

    counts: dict[str, int] = {}
    for result in manifest.get("daily_results") or []:
        if not isinstance(result, dict):
            continue
        dataset_date = str(result.get("dataset_date", ""))
        split_rows = result.get("split_rows") or {}
        train_rows = int(split_rows.get("train", 0))
        if dataset_date and train_rows > 0:
            counts[dataset_date] = train_rows
    if not counts:
        raise ValueError(
            "--daily-equal-weighting requires manifest.daily_results train counts"
        )
    total = sum(counts.values())
    weights = {
        dataset_date: total / (len(counts) * rows)
        for dataset_date, rows in counts.items()
    }
    weighted_mean = sum(
        counts[dataset_date] * weights[dataset_date]
        for dataset_date in counts
    ) / total
...
def load_bc_initialization(
    ...
    cross_feature_init = feature_version != FEATURE_VERSION
    if cross_feature_init and feature_version not in allowed_source_feature_versions:
        raise ValueError(
            "--init-checkpoint must be a BC checkpoint with feature_version "
            f"{FEATURE_VERSION!r}; got {feature_version!r}. Explicitly allow "
            "an architecture-compatible transfer with "
            "--allow-init-feature-version."
        )
    ...
    return {
        ...
        "cross_feature_initialization": cross_feature_init,
    }
...
def configure_trainable_scope(
    ...
    *,
    ...
    """Apply a fail-closed BC parameter scope and return its audit."""
    ...
    all_names = tuple(name for name, _ in named_parameters)
    ...
    if scope == "all":
        ...
        missing_actor_names = sorted(
            set(TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES) - available_names
        )
        if missing_actor_names:
            raise RuntimeError(
                "actor6 scope is missing required actor parameter keys: "
                f"{missing_actor_names}"
            )
        selected_names = tuple(
            name
            for name in all_names
            if name in TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES
        )
        if set(selected_names) != set(
            TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES
        ) or len(selected_names) != len(TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES):
            raise RuntimeError("actor6 scope did not select exactly actor6")
    else:
        available_names = set(all_names)
        missing_actor_names = sorted(
            set(TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES) - available_names
        ...
        if missing_actor_names:
            raise RuntimeError(
                "transformer_actor scope is missing required actor parameter "
                f"keys: {missing_actor_names}"
            )
        transformer_names = tuple(
            name for name in all_names if name.startswith("transformer.")
        )
        if not transformer_names:
            raise RuntimeError(
                "transformer_actor scope found no transformer.* parameters"
            )
        selected_name_set = set(transformer_names) | set(
            TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES
        )
        selected_names = tuple(
            name for name in all_names if name in selected_name_set
        )
        ...
        if selected_actor_names != set(
            TRANSFORMER_ACTOR_HEAD_PARAMETER_NAMES
        ):
            raise RuntimeError(
                "transformer_actor scope did not select exactly actor6"
            )
    ...
    if observed_names != selected_names:
        raise RuntimeError(
            "requires_grad audit differs from the selected parameter keys"
        )
    ...
    if not trainable_parameters:
        raise RuntimeError(
            f"Trainable scope {scope!r} produced no optimizer parameters"
        )
    ...
    audit = {
        ...
        "trainable_parameter_names": list(observed_names),
        "frozen_tensor_count": (
            len(named_parameters) - len(trainable_parameters)
        ),
    ...
    if (
        expected_trainable_tensors is not None
        and len(trainable_parameters) != expected_trainable_tensors
    ):
        mismatches["trainable_tensor_count"] = {
            "observed": len(trainable_parameters),
            "expected": expected_trainable_tensors,
        }
    if (
        expected_trainable_parameters is not None
        and trainable_parameter_count != expected_trainable_parameters
    ):
        mismatches["trainable_parameter_count"] = {
            "observed": trainable_parameter_count,
            "expected": expected_trainable_parameters,
        }
    if mismatches:
        raise ValueError(f"Trainable parameter audit mismatch: {mismatches}")
    return trainable_parameters, audit
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Feature contract of a pre-featurized Parquet dataset and output "
            "checkpoint. Raw JSONL data must use the trainer's native v6 "
            "featurizer."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Multiply train-only actor/value row weights so every dataset "
            "date contributes equal total mass. Requires daily_results in "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "halfway back to uniform; 'focus' upweights only the team named "
            "by --policy-team-focus-name. Value BCE remains uniformly "
            "weighted."
        ),
    )
    parser.add_argument(
        "--policy-team-focus-name",
        help="Exact train-row team_name used by --policy-team-balance focus.",
    )
    parser.add_argument(
        "--policy-team-focus-relative-weight",
        type=float,
        default=1.25,
        help=(
            "Unnormalized relative actor-loss weight for the focus team; "
            "all row weights are rescaled to train-row mean one."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train-select-context",
        action="append",
        type=int,
        default=[],
        help=(
            "Optional context filter applied only to optimizer/training rows; "
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Explicitly allow an architecture-compatible initialization "
            "checkpoint from this different feature version. Repeatable; "
            "the transfer is recorded in run_config.json."
        ),
    )
    parser.add_argument(
        "--trainable-scope",
        ...
        default="all",
        help=(
            "Parameters updated by BC. 'all' preserves historical behavior; "
            "'transformer_actor' trains transformer.* and exactly actor6; "
            "'actor6' trains only the six named actor-head tensors."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        help=(
            "Optional fail-closed assertion for the number of optimizer "
            "parameter tensors."
        ),
    )
    parser.add_argument(
        ...
        type=int,
        help=(
            "Optional fail-closed assertion for the number of trainable "
            "scalar parameters."
        ),
    )
    parser.add_argument(
        "--split-mode",
        choices=("archive", "episode_hash", "all_train"),
        default="archive",
    )
    parser.add_argument(
        "--skip-valid",
        action="store_true",
        help=(
            "Train for the requested fixed epoch count without reading a "
            "validation split. The final epoch is written to both best.pt "
            "and last.pt; target-accuracy gates are disabled."
        ),
    )
    ...
    if args.data.is_dir() and not (args.data / "manifest.json").is_file():
        raise FileNotFoundError(args.data / "manifest.json")
    ...
    if args.policy_team_balance == "focus":
        if not args.policy_team_focus_name:
            raise ValueError(
                "--policy-team-balance focus requires "
                "--policy-team-focus-name"
            )
    elif args.policy_team_focus_name is not None:
        raise ValueError(
            "--policy-team-focus-name requires --policy-team-balance focus"
        )

    manifest = read_manifest(args.data)
    daily_weights = (
        daily_equal_weights_from_manifest(manifest)
        if args.daily_equal_weighting
        else None
    )
    stored_feature_version = manifest.get("feature_version")
    if (
        stored_feature_version is not None
        and stored_feature_version != FEATURE_VERSION
    ):
        raise ValueError(
            "Dataset feature version does not match --feature-version: "
            f"dataset={stored_feature_version!r} requested={FEATURE_VERSION!r}"
        )
    if args.data.is_file() and FEATURE_VERSION != (
        "ptcg-bc-orbit-entity-transformer-v6-priority"
    ):
        raise ValueError(
            "Raw ZIP training only supports the native v6-priority "
            "featurizer; convert with the requested feature module first"
        )
    expected_train_rows = (
    ...
    if args.policy_team_balance != "none":
        ...
        elif args.policy_team_balance == "focus":
            assert args.policy_team_focus_name is not None
            policy_team_weights = focus_team_weights(
                policy_team_counts,
                args.policy_team_focus_name,
                args.policy_team_focus_relative_weight,
            )
        else:
    ...
    config = TrainConfig(
        ...
        deck_hashes=tuple(args.deck_hash),
        team_names=tuple(args.team_name),
        train_select_contexts=tuple(sorted(set(args.train_select_context))),
        ...
        policy_team_focus_name=args.policy_team_focus_name,
        policy_team_focus_relative_weight=(
            args.policy_team_focus_relative_weight
        ),
        init_checkpoint=(
        ...
        expected_trainable_parameters=args.expected_trainable_parameters,
        daily_equal_weighting=args.daily_equal_weighting,
    ...
    initialization = None
    if args.init_checkpoint is not None:
        initialization = load_bc_initialization(
            model,
            args.init_checkpoint,
            tuple(args.allow_init_feature_version),
        )
        log(
            "initialized_from_bc="
            f"{initialization['path']} sha256={initialization['sha256']}"
        )
    trainable_parameters, parameter_audit = configure_trainable_scope(
        model,
        config.trainable_scope,
        expected_trainable_tensors=config.expected_trainable_tensors,
        expected_trainable_parameters=config.expected_trainable_parameters,
    )
    ...
    parameter_count = parameter_audit["total_parameter_count"]
    if config.policy_team_balance == "none":
        ...
        if config.policy_team_balance == "sqrt_clip2":
            formula = "clip(z*sqrt(mean_train_team_rows/team_rows),0.5,2.0)"
            strength_relative_to_sqrt_clip2: float | None = 1.0
        elif config.policy_team_balance == "sqrt_clip2_half":
            formula = (
                "1+0.5*(clip(z*sqrt("
                "mean_train_team_rows/team_rows),0.5,2.0)-1)"
            )
            strength_relative_to_sqrt_clip2 = 0.5
        elif config.policy_team_balance == "focus":
            formula = (
                "z*(focus_relative_weight if team_name==focus_team else 1)"
            )
            strength_relative_to_sqrt_clip2 = None
        else:
            raise AssertionError("validated policy-team balance mode was not handled")
        policy_team_balance_info = {
            ...
            "focus_team_name": config.policy_team_focus_name,
            "focus_team_relative_weight": (
                config.policy_team_focus_relative_weight
                if config.policy_team_balance == "focus"
                else None
            ),
            "train_only": True,
    ...
    run_info = {
        ...
        "validation": {
            "skipped": args.skip_valid,
            "checkpoint_selection": (
                "fixed_final_epoch" if args.skip_valid else "best_valid_exact"
            ),
        },
    }
    ...
    log(
        ...
        f"trainable_tensors={parameter_audit['trainable_tensor_count']:,} "
        "trainable_parameters="
        f"{parameter_audit['trainable_parameter_count']:,} "
    ...
    with metrics_path.open("w", encoding="utf-8") as metrics_file:
        for epoch in range(1, config.epochs + 1):
            ...
            if (
                args.expected_train_rows is not None
                and train_metrics["rows"] != expected_train_rows
            ):
                raise RuntimeError(
                    "training epoch row count differs from "
                    f"--expected-train-rows: epoch={epoch} "
                    f"observed={train_metrics['rows']} "
                    f"expected={expected_train_rows}"
                )
            valid_metrics = None
            ...
            if args.skip_valid:
                ...
                log(
                    f"epoch={epoch} validation_skipped=true "
                    "checkpoint_selection=fixed_final_epoch"
                )
            else:
                assert valid_metrics is not None
    ...
    summary = {
        ...
        "validation_skipped": args.skip_valid,
        "checkpoint_selection": (
            "fixed_final_epoch" if args.skip_valid else "best_valid_exact"
        ),
        "target_reached_on_valid": (
            False if args.skip_valid else best_accuracy >= config.target_accuracy
        ),
