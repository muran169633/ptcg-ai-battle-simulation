...
parent_ppo_sha256="77d17fb3c281e93a35386722a4400a119823910d28d8367d7ec163de0f0f69fa"
learner_deck="$pool_dir/decks/rank01_16380946_07bedfffbfad.csv"
bc_replay="$repo_dir/data/dragapult_current_07bed_20260812_v1/dragapult_07bed_aug12.zip"
bc_replay_sha256="a67cad9ab1382706d79a7ba00e40f7ed332b8f801e37c5fae54399c819faa60a"
output_dir="$repo_dir/artifacts/dragapult_standard_parallel_20260814_v1/ppo_currenttop23_90pool10self_gate58_mb4096_u120_seed2026081407"
...
actual_parent_sha256="$(sha256sum "$parent_ppo" | cut -d' ' -f1)"
if [[ "$actual_parent_sha256" != "$parent_ppo_sha256" ]]; then
  echo "Dragapult parent checkpoint hash mismatch: $actual_parent_sha256" >&2
  exit 1
fi
actual_replay_sha256="$(sha256sum "$bc_replay" | cut -d' ' -f1)"
if [[ "$actual_replay_sha256" != "$bc_replay_sha256" ]]; then
  echo "Dragapult exact-deck replay hash mismatch: $actual_replay_sha256" >&2
  exit 1
fi
