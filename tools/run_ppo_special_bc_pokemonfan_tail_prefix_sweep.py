#!/usr/bin/env python3
"""Reconstruct and publish the P33-P36 prefixes of the frozen tail5 path.
...
parent, and the original specialist archive.  Training executes the exact
five-step P37 trajectory, publishes only the first four prefix checkpoints,
and requires the in-memory fifth step to reproduce the already frozen P37
model and replay-optimizer hashes.  Audit-only mode constructs and hashes the
complete replay cache but performs zero optimizer steps and writes no
checkpoint.
"""
...
OUTPUT_SCHEMA = "ptcg-u456-post-pokemonfan32-tail-prefix-sweep-v1"
...
PUBLISHED_PREFIX_STEPS = (1, 2, 3, 4)
EXPECTED_P37_MODEL_SHA256 = (
    "bb84c5ad3f7baefd4d44d0edaf73d27e164f981b7d8cce0902df8d7c1acd4a45"
)
EXPECTED_P37_REPLAY_SHA256 = (
    "6b4df5a527aab49e2dabcab993e2af9b5a89bb60297f381c6c08e02f4d4bb445"
)
SWEEP_DESIGN_PATH = (
    REPO_ROOT
    / "artifacts/ppo_u456inc_generalbc_ppo_specialbc_"
    "pokemonfan32_tailprefix_sweep_reconstruct_seed202608030."
    "design_preregistration.json"
)
SWEEP_DESIGN_SHA256 = (
    "cef23090d1abbed49b6c6f250665f3c5c1b843155665a3b90076679b7d82beb2"
)
...
def clone_nested_to_cpu(value: Any) -> Any:
    """Clone nested optimizer state without retaining live CUDA storage."""
...
def main() -> None:
    ...
    for path, expected, label in (
        (executor_path, args.expected_executor_sha256, "tail-prefix executor"),
        ...
        (SWEEP_DESIGN_PATH, SWEEP_DESIGN_SHA256, "sweep design preregistration"),
    ...
    for special_step, batch_index in enumerate(args.batch_indices, start=1):
        ...
        if special_step in PUBLISHED_PREFIX_STEPS:
            ...
            expected_endpoint_step = EXPECTED_REPLAY_STEP_BEFORE + special_step
            if (
                len(endpoint_changed_names) != EXPECTED_ACTOR_TENSORS
                or set(endpoint_changed_names) != set(actor_names)
                or len(endpoint_replay_steps) != EXPECTED_ACTOR_TENSORS
                or set(endpoint_replay_steps) != {expected_endpoint_step}
                ...
                raise RuntimeError(
                    f"P{P32_STEPS + special_step} prefix integrity failed"
                )
            prefix_snapshots.append({
                "additional_steps": special_step,
                "cumulative_special_bc_steps": P32_STEPS + special_step,
                ...
                "replay_state_sha256": repair_audit.nested_sha256(
                    endpoint_replay_state
                ),
    ...
    if (
        model_hash_after != EXPECTED_P37_MODEL_SHA256
        or replay_hash_after != EXPECTED_P37_REPLAY_SHA256
    ):
        raise RuntimeError(
            "Step-5 equality sentinel did not reproduce the frozen P37 path"
        )
    if len(prefix_snapshots) != len(PUBLISHED_PREFIX_STEPS):
        raise RuntimeError("The four required prefix snapshots are incomplete")
    ...
    trajectory_integrity = {
        ...
        "p37_model_state_sha256_expected": EXPECTED_P37_MODEL_SHA256,
        "p37_replay_state_sha256_expected": EXPECTED_P37_REPLAY_SHA256,
        "step5_matches_frozen_p37_model_and_replay": True,
    }
    endpoint_records: list[dict[str, Any]] = []
    for snapshot in prefix_snapshots:
        additional_steps = int(snapshot["additional_steps"])
        cumulative_steps = int(snapshot["cumulative_special_bc_steps"])
        endpoint_name = f"p{cumulative_steps}"
        endpoint_special = copy.deepcopy(common["special_bc"])
        endpoint_special.update({
            "additional_steps": additional_steps,
            "cumulative_special_bc_steps": cumulative_steps,
            "batch_indices": args.batch_indices[:additional_steps],
            "batch_sha256": batch_hashes[:additional_steps],
            "trajectory_reconstruction_seed": args.seed,
        })
        endpoint_integrity = {
            "optimizer_steps": additional_steps,
            "additional_steps": additional_steps,
            "cumulative_special_bc_steps": cumulative_steps,
            "rows": additional_steps * SPECIAL_BATCH_SIZE,
            "context34_rows": additional_steps,
            "all_sample_weights_one": True,
            "all_metrics_and_model_tensors_finite": True,
            "model_state_sha256_before": model_hash_before,
        ...
        endpoint_provenance = common | {
            "status": "tail_prefix_endpoint_completed",
            "special_bc": endpoint_special,
            "per_step": per_step[:additional_steps],
            "integrity": endpoint_integrity,
            "trajectory_sentinel": {
                "full_steps_executed_in_memory": SPECIAL_STEPS,
                "p37_model_state_sha256": model_hash_after,
                "p37_replay_state_sha256": replay_hash_after,
                "exact_match": True,
                "p37_checkpoint_published": False,
            },
            "checkpoint_update_label": 456,
            "not_a_new_ppo_update": True,
        }
        output_checkpoint = output_dir / (
            f"special-bc-pokemonfan32-tail-prefix-{cumulative_steps:04d}.pt"
        )
        ...
        endpoint_records.append({
            ...
            "update": 456,
            "additional_special_bc_steps": additional_steps,
            "cumulative_special_bc_steps": cumulative_steps,
            "model_state_sha256": snapshot["model_state_sha256"],
            "replay_state_sha256": snapshot["replay_state_sha256"],
            "replay_adamw_step": EXPECTED_REPLAY_STEP_BEFORE + additional_steps,
        })

    result = common | {
        "status": "tail_prefix_sweep_completed",
        "per_step": per_step,
        ...
        "p37_checkpoint_writes": 0,
        "checkpoint_update_label": 456,
        "not_a_new_ppo_update": True,
    }
    write_json(output_dir / "tail_prefix_sweep_manifest.json", result)
    expected_files = [
        f"special-bc-pokemonfan32-tail-prefix-{step:04d}.pt"
        for step in range(33, 37)
    ] + ["tail_prefix_sweep_manifest.json"]
    if sorted(path.name for path in output_dir.iterdir()) != sorted(expected_files):
        raise RuntimeError("Tail-prefix output directory contains unexpected files")
    print(json.dumps(result, ensure_ascii=False, sort_keys=True))
