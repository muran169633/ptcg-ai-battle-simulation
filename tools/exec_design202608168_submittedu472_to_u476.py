#!/usr/bin/env python3
"""Audit or execute submitted-U472 -> U476 short multi-opponent PPO."""
...
SCHEMA = "ptcg-design202608168-submitted-u472-to-u476-parentguard-v1"
SEED = 202608168
TRAINER = TOOLS / "train_ppo.py"
TRAINER_SHA256 = "de3d1467d2b18a551b66c2e897e774d251a898b276dac709268b03d05da41ba3"
PARENT = ROOT / "artifacts/ppo_guardedu468_updatedreplay_g8_ppo4x96_u472_freshjointactor6_s8_design202608148/ppo_stage/B_gold_league/seed-202608148/checkpoints/update-0472.pt"
...
OUTPUT_ROOT = ROOT / "artifacts/design202608168_submittedu472_ppo4x192_u476"
OUTPUT_DIR = OUTPUT_ROOT / "B_gold_league/seed-202608168"
TERMINAL = OUTPUT_DIR / "checkpoints/update-0476.pt"
ATTEMPT = ROOT / ".ptcg-design202608168-submittedu472-u476-attempt.json"
LOG = ROOT / "artifacts/design202608168_submittedu472_ppo4x192_u476.log"
DEPENDENCIES = {
...
def derive_command() -> list[str]:
    ...
    command.extend(["--extra-opponent", str(PARENT), str(DECK)])
    command.extend([
        "--output-dir", str(OUTPUT_DIR), "--updates", "476",
        ...
        "--schedule-start-update", "473", "--gamma", "1.0", "--gae-lambda", "0.97",
        ...
        "--trainable-scope", "heads", "--bc-kl-start", "0.006",
        "--bc-kl-end", "0.004", "--target-kl", "0.002",
        ...
        "--snapshot-interval", "1000000", "--max-pool-size", "14",
        ...
        "--bc-replay-lr-scale", "0.025", "--bc-replay-loss", "ordered",
    ...
    for value in values:
        command.extend([
            "--opponent-base-quota",
            opponent_name(value["checkpoint"], value["deck"]),
            str(GOLD_QUOTAS[value["policy_id"]]),
        ])
    command.extend([
        "--opponent-base-quota", opponent_name(PARENT, DECK), str(PARENT_QUOTA)
    ])
    return command
...
def bindings(protocol: Path) -> dict[Path, str]:
    result = {
        ...
        TRAINER: TRAINER_SHA256, PARENT: PARENT_SHA256, BC: BC_SHA256,
        REPLAY: REPLAY_SHA256, DECK: DECK_SHA256, LEAGUE: LEAGUE_SHA256,
        **DEPENDENCIES,
    }
...
def expected_protocol(command: list[str]) -> dict[str, Any]:
    return {
        ...
        "parent_guard_quota": PARENT_QUOTA,
        "hyperparameters": {
            "actor_learning_rate": 4.8e-5,
...
def validate(protocol: Path, expected_protocol_sha: str) -> tuple[dict[str, Any], list[str], dict[str, Any]]:
    ...
    if sum(GOLD_QUOTAS.values()) != 160 or PARENT_QUOTA != 32:
        raise RuntimeError("quota sum contract failed")
    if any(value % 2 for value in [*GOLD_QUOTAS.values(), PARENT_QUOTA]):
        raise RuntimeError("all quotas must support exact seat balance")
    ...
    return value, command, {
        ...
        "fixed_quota_sum": sum(GOLD_QUOTAS.values()) + PARENT_QUOTA,
        "targets_absent": True,
        "optimizer_steps": 0,
        "rollout_games": 0,
    }
...
def run(protocol_path: Path, protocol: dict[str, Any], command: list[str]) -> int:
    ...
    try:
        os.write(fd, audit.canonical_json_bytes(marker))
        os.fsync(fd)
    finally:
        os.close(fd)
    ...
    try:
        ...
        with os.fdopen(log_fd, "wb", buffering=0) as handle:
            ...
            handle.write(audit.canonical_json_bytes(marker | {
...
def main() -> int:
    ...
    if args.print_expected_protocol:
        print(json.dumps(expected_protocol(derive_command()), indent=2, sort_keys=True))
        return 0
    protocol, command, report = validate(
        args.protocol.resolve(), args.expected_protocol_sha256
    )
    if args.audit_only:
