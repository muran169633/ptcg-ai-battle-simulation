#!/usr/bin/env python3
"""Build only the one-shot CW11 specialist dev-consistency preregistration.

This generator never evaluates a panel and never creates a broad template.
The generated document binds the evaluation-only CW11 checkpoint, its
materialization manifest/completion/materializer, raw U468, the executor,
evaluator/dependencies, three frozen data archives, six raw baselines, all 60
gates, and six exactly-once commands.

The specialist-valid rows were consumed during CW11 construction.  Therefore
the generated protocol is dev consistency only, never promotion evidence, and
cannot authorize broad, Gold, packaging, upload, or submission.
"""
...
TOOLS = ROOT / "tools"
SCRIPT = TOOLS / "build_cw11_specialist_dev_consistency_preregistration_v1.py"
EXPECTED_PYTHON = Path("/home/xxc/miniconda3/envs/my_project_env/bin/python")
FROZEN_MODE = 0o555

EXECUTOR = TOOLS / "run_cw11_single_endpoint_specialist_dev_consistency_v1.py"
EXECUTOR_SHA256 = "6d96258973a00b78405e4446bf38a88fc9a862d0b5862af47f2a359c8579b0dc"
...
def import_executor() -> tuple[ModuleType, dict[str, Any]]:
    _, evidence = read_frozen_source(EXECUTOR, EXECUTOR_SHA256, "CW11 dev executor")
    spec = importlib.util.spec_from_file_location(
        f"_cw11_dev_executor_{EXECUTOR_SHA256[:12]}", EXECUTOR
    )
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot import CW11 dev executor")
...
def require_targets_absent(*, include_preregistration: bool) -> dict[str, bool]:
    ...
    if not all(absent.values()):
        ...
        raise FileExistsError(f"fresh CW11 dev targets must be absent: {present}")
    return absent
...
def build_document() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    provenance_evidence, _ = executor.verify_materialization_provenance(
        provenance_bindings
    )
    ...
    executor_binding = binding(EXECUTOR, "CW11 dev executor") | {
        "python": str(EXPECTED_PYTHON),
        "flags": ["-I", "-B"],
    }
    if executor_binding["sha256"] != EXECUTOR_SHA256:
        raise RuntimeError("CW11 dev executor SHA drift")
...
def main() -> None:
    ...
    self_payload, self_evidence = legacy.read_plain_file(
        SCRIPT, "CW11 dev preregistration generator"
    )
    del self_payload
    ...
    result = {
        "schema_version": "ptcg-cw11-specialist-dev-consistency-builder-result-v1",
        "status": "static_zero_write_audit_only",
        "self": self_evidence,
        "executor": EXECUTOR_EVIDENCE,
        "candidate": audit["candidate"],
        "materialization_provenance": audit["materialization_provenance"],
