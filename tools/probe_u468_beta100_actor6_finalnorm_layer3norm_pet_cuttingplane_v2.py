#!/usr/bin/env python3
"""Thin frozen extension of combined v1 with complete layer-3 norms.

The mutable vector is the frozen actor6 prefix, final LayerNorm, and both
LayerNorms of transformer layer 3: twelve FP32 tensors / 66,561 scalars.
Only the actor6 prefix is projected out of the frozen P/E/T span.  The hard
combined L2 radius remains 1e-3 and every weak-BF16, Direct512, complete-train,
consumer, and restoration rule is delegated to hash-bound combined v1.
...
TOOLS = ROOT / "tools"
SCRIPT = TOOLS / "probe_u468_beta100_actor6_finalnorm_layer3norm_pet_cuttingplane_v2.py"
EXPECTED_PYTHON = Path("/home/xxc/miniconda3/envs/my_project_env/bin/python")
SCHEMA = "ptcg-u468-beta100-actor6-finalnorm-layer3norm-pet-cuttingplane-v2"
...
LAYER3NORM_ELEMENTS = 512
NORM_ELEMENTS = FINALNORM_ELEMENTS + LAYER3NORM_ELEMENTS
...
EXPECTED_BETA_VARIABLE12_SHA256 = (
    "4b95974ed4d280bce1df0833abf3465e47a461742864331f189e6f4182847025"
)
EXPECTED_BETA_OTHER68_SHA256 = (
    "e8ce1a9a80897d3175356e1475ff11f7b2d7edc4765502271e8357bbc1da7abb"
)
...
NORM_NAMES = FINAL_NORM_NAMES + LAYER3_NORM_NAMES
...
def validate_runtime() -> None:
    ...
    if len(VARIABLE_NAMES) != 12 or COMBINED_ELEMENTS != 66561:
        raise RuntimeError("v2 variable manifest drift")
    if HARD_COMBINED_L2 != BASE.HARD_COMBINED_L2:
        raise RuntimeError("v2 attempted to relax the combined radius")


def endpoint_audit_v2(vector: np.ndarray, q: np.ndarray) -> dict[str, Any]:
    value = np.asarray(vector, dtype=np.float64)
    if value.shape != (COMBINED_ELEMENTS,):
        raise RuntimeError(f"v2 endpoint shape drift: {value.shape}")
    if q.shape != (COMBINED_ELEMENTS, 3):
        raise RuntimeError(f"v2 embedded-Q shape drift: {q.shape}")
    components = q.T @ value
    actor = value[:ACTOR_ELEMENTS]
    finalnorm = value[ACTOR_ELEMENTS : ACTOR_ELEMENTS + FINALNORM_ELEMENTS]
    layer3norm = value[ACTOR_ELEMENTS + FINALNORM_ELEMENTS :]
    combined_l2 = float(np.linalg.norm(value))
    actor_l2 = float(np.linalg.norm(actor))
    finalnorm_l2 = float(np.linalg.norm(finalnorm))
    layer3norm_l2 = float(np.linalg.norm(layer3norm))
    ...
    return {
        ...
        "layer3norm_l2": layer3norm_l2,
        "all_norms_l2": float(math.hypot(finalnorm_l2, layer3norm_l2)),
...
def build_combined_geometry_v2(basis: Mapping[str, Any]) -> dict[str, Any]:
    q_actor = np.ascontiguousarray(basis["q"].numpy(), dtype=np.float64)
    if q_actor.shape != (ACTOR_ELEMENTS, 3):
        raise RuntimeError("v2 actor PET Q shape drift")
    ...
    if residual > 1.0e-12 or bool(np.any(q[ACTOR_ELEMENTS:] != 0.0)):
        raise RuntimeError("v2 embedded PET geometry drift")
    ...
    checks = {
        "variable_tensor_count_exact_12": len(variable) == 12,
        "combined_elements_exact_66561": (
        ...
        "other_tensor_count_exact_68": len(other) == 68,
        "variable12_sha_exact": (
        ...
        "embedded_norm_rows_exact_zero": bool(
            np.all(q[ACTOR_ELEMENTS:] == 0.0)
        ),
    }
    if not all(checks.values()):
        raise RuntimeError(f"v2 beta100 state geometry drift: {checks}")
    return {
        "q": q,
        "q_actor": q_actor,
        "report": {
            ...
            "layer3norm_elements": LAYER3NORM_ELEMENTS,
            "all_norm_elements": NORM_ELEMENTS,
            ...
            "pet_span": "frozen actor6 P/E/T with 768 zero norm rows",
            "beta_variable12_sha256": EXPECTED_BETA_VARIABLE12_SHA256,
            "beta_other68_sha256": EXPECTED_BETA_OTHER68_SHA256,
            "checks": checks,
        },
    }
...
def _binding_values() -> dict[str, Any]:
    return {
        ...
        "COMBINED_ELEMENTS": COMBINED_ELEMENTS,
        "EXPECTED_BETA_VARIABLE8_SHA256": EXPECTED_BETA_VARIABLE12_SHA256,
        "EXPECTED_BETA_OTHER72_SHA256": EXPECTED_BETA_OTHER68_SHA256,
        "build_combined_geometry": build_combined_geometry_v2,
        "endpoint_audit": endpoint_audit_v2,
    }
...
def static_audit() -> dict[str, Any]:
    ...
    checks = {
        ...
        "scope_exact_12_tensors_66561": (
            len(VARIABLE_NAMES) == 12 and COMBINED_ELEMENTS == 66561
        ),
        "norm_partition_exact_256_plus_512": (
            FINALNORM_ELEMENTS == 256
            and LAYER3NORM_ELEMENTS == 512
            and NORM_ELEMENTS == 768
        ),
        "one_hard_combined_radius_unchanged": HARD_COMBINED_L2 == 1.0e-3,
        "solver_self_test_under_v2_bindings": solver_test["pass"],
    ...
    return {
        "status": "static_layer3norm_v2_audit_passed",
        "checks": checks,
...
def _normalize_result_labels(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(scope, dict):
        scope["other_state_tensors_exact"] = 68
        scope["finalnorm_elements"] = FINALNORM_ELEMENTS
        scope["layer3norm_elements"] = LAYER3NORM_ELEMENTS
        scope["all_norm_elements"] = NORM_ELEMENTS
    integrity = result.get("final_integrity")
    if isinstance(integrity, dict):
        if "variable8_tensor_restored_exact" in integrity:
            integrity["variable12_tensor_restored_exact"] = integrity.pop(
                "variable8_tensor_restored_exact"
            )
        if "other72_sha_after" in integrity:
            integrity["other68_sha_after"] = integrity.pop("other72_sha_after")
        if "other72_restored_exact" in integrity:
            integrity["other68_restored_exact"] = integrity.pop(
...
def run_probe(
    ...
    def enriched_consumer(payload_source: Mapping[str, Any]) -> None:
        if candidate_consumer is None:
            raise RuntimeError("internal v2 consumer contract drift")
        ...
        if int(vector.numel()) != COMBINED_ELEMENTS:
            raise RuntimeError("v2 consumer vector shape drift")
        final = vector[ACTOR_ELEMENTS : ACTOR_ELEMENTS + FINALNORM_ELEMENTS]
        layer3 = vector[ACTOR_ELEMENTS + FINALNORM_ELEMENTS :]
        payload["actual_finalnorm_delta_l2"] = float(final.double().norm())
        payload["actual_layer3norm_delta_l2"] = float(layer3.double().norm())
        payload["actual_all_norms_delta_l2"] = float(
            vector[ACTOR_ELEMENTS:].double().norm()
        )
        payload["v2_variable_names"] = list(VARIABLE_NAMES)
...
def cache_audit() -> dict[str, Any]:
    ...
    result["status"] = "layer3norm_v2_cache_audit_passed_zero_write_train_only"
    result["self"] = read_regular_bytes(SCRIPT, None, "layer3norm v2 self")[1]
...
def main() -> None:
    ...
    if args.mode == "static-audit":
        result: dict[str, Any] = {
            ...
            "self": read_regular_bytes(SCRIPT, None, "layer3norm v2 self")[1],
