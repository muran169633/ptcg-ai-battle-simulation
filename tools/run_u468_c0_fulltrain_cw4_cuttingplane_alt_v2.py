...
SCRIPT = TOOLS / "run_u468_c0_fulltrain_cw4_cuttingplane_alt_v2.py"
SCHEMA = "ptcg-u468-c0-fulltrain-cw4-cuttingplane-formal-alt-v2"
SEED = 202608205
...
def canonical_to_internal_pairs(
    ...
    identity_role_records: list[dict[str, Any]] = []
    for ordinal, record in enumerate(ledger):
        ...
        resolved_descriptor = descriptors[resolved_row_index]
        checks = {
            "full_identity_resolved": identity
            == (
                str(resolved_descriptor["panel"]),
                str(resolved_descriptor["member"]),
                int(resolved_descriptor["line_index_zero_based"]),
                str(resolved_descriptor["line_sha256"]),
            ),
            "resolved_original_row_lt_19": resolved_row_index < 19,
            "canonical_key_row_matches_resolved": key[0] == resolved_row_index,
            "record_role_matches_resolved_descriptor": str(record["role"])
            == str(resolved_descriptor["role"]),
        }
        if not all(checks.values()):
            raise RuntimeError(
                f"canonical active pair identity/role remap drift: {checks}"
            )
        identity_role_records.append(
            {
                "ordinal": ordinal,
                "key": key,
                "identity": copy.deepcopy(record_identity),
                "resolved_row_index": resolved_row_index,
                "record_role": str(record["role"]),
                "resolved_descriptor_role": str(resolved_descriptor["role"]),
                "checks": checks,
                "pass": True,
            }
        )
    ...
    audit = {
        "record_count": len(identity_role_records),
        "pass_count": sum(bool(record["pass"]) for record in identity_role_records),
        "all_20_identity_and_role_exact": (
            len(identity_role_records) == EXPECTED_ORIGINAL_ACTIVE_COUNT
            and all(bool(record["pass"]) for record in identity_role_records)
        ),
        "records": identity_role_records,
    }
    if not bool(audit["all_20_identity_and_role_exact"]):
        raise RuntimeError(f"active20 identity/role audit failed: {audit}")
    return pairs, audit
...
def continue_from_c0(
    ...
    try:
        ...
        for row_index in range(19, 23):
            ...
            first_stage = int(descriptor["first_different_ranking_stage"])
            threat_order = [int(value) for value in descriptor["formal_threat_order"]]
            checks = {
                ...
                "context_exact_expected": int(metadata["context"])
                == int(descriptor["expected_context"]),
                "first_difference_stage_zero": first_stage == 0,
                "first_difference_stage_in_threat_bounds": 0
                <= first_stage
                < len(threat_order),
                "negative_is_threat_at_first_difference_stage": pair_options[1]
                == threat_order[first_stage],
            }
            if not all(checks.values()):
                raise RuntimeError(
            ...
            new_row_schema_audit.append(
                {
                    ...
                    "expected_context": int(descriptor["expected_context"]),
                    "expert_order": expert_order,
                    "option_count": option_count,
                    "pair_options": list(pair_options),
                    "first_different_ranking_stage": first_stage,
                    "formal_threat_order": threat_order,
                    "checks": checks,
                }
            )
        ...
        active_pairs, active20_identity_role_audit = canonical_to_internal_pairs(
        ...
        return {
            ...
            "active20_identity_role_audit": active20_identity_role_audit,
