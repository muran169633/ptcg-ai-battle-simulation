#!/usr/bin/env python3
"""CW24 v9b frozen-v1/v6 spherical-midpoint linear preflight.
...
TOOLS = ROOT / "tools"
SCRIPT = TOOLS / "probe_u468_cw11_v1_v6_spherical_midpoint_preflight_cw24_v9b.py"
OUTPUT = ROOT / "artifacts/cw24_cw11_v1_v6_spherical_midpoint_preflight_cw24_v9b.json"
EXPECTED_PYTHON = Path("/home/xxc/miniconda3/envs/my_project_env/bin/python")
SCHEMA = "ptcg-u468-cw11-v1-v6-spherical-midpoint-linear-preflight-cw24-v9b"
SEED = 202608049
R_SAFE = 0.000999975
...
class ProtocolError(RuntimeError):
    """Fail-closed CW24 v9b preflight error."""
...
def self_evidence(require_frozen: bool) -> dict[str, Any]:
    ...
    if require_frozen and not all(checks.values()):
        raise ProtocolError(f"v9b source audit failed: {checks}")
    return {
...
def publish_o_excl(path: Path, payload: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written <= 0:
                raise ProtocolError("short v9b result write")
            offset += written
        os.fsync(fd)
        os.fchmod(fd, 0o444)
        os.fsync(fd)
    ...
    if not all(checks.values()):
        raise ProtocolError(f"v9b result publication drift: {checks}")
    return {
        "path": str(path.relative_to(ROOT)),
...
'''
    new = '''        if not candidate_pass:
            V9B_CAPTURE_V1_FAILED_ACTUAL_DELTA(actual_delta.copy(), trial)
            return {
'''
    audits: list[dict[str, Any]] = []
    source = splice_once(source, old, new, "capture_frozen_v1_failed_actual_delta", audits)
    transformed = source.encode()
    name = "cw24_v9b_transformed_frozen_v1"
    if name in sys.modules:
        raise ProtocolError("v9b transformed v1 module name occupied")
    module = types.ModuleType(name)
    module.__file__ = str(V1_SOURCE)
    module.__package__ = ""
    module.__dict__["V9B_CAPTURE_V1_FAILED_ACTUAL_DELTA"] = capture
    ...
    checks = {
        ...
        "callback_identity_exact": module.V9B_CAPTURE_V1_FAILED_ACTUAL_DELTA is capture,
    }
    if not all(checks.values()):
...
'''
    ...
    baseline_new = '''            if trial_evaluations == 0 and accepted_iterates == 0:
                V9B_CAPTURE_V4_BASELINE_LINEARIZATION(
                    matrix.copy(),
                    rhs.copy(),
...
'''
    ...
    s2_new = '''            iteration_log.append(log_entry)
            if trial_evaluations == 2 and accepted_iterates == 1 and not accept:
                V9B_CAPTURE_V6_REJECTED_S2_ACTUAL_DELTA(
                    actual_trial_x.copy(),
                    {
...
'''
    ...
    transformed = source.encode()
    name = "cw24_v9b_transformed_frozen_v4_v6"
    if name in sys.modules:
        raise ProtocolError("v9b transformed v4/v6 module name occupied")
    module = types.ModuleType(name)
    module.__file__ = str(V4_SOURCE)
    module.__package__ = ""
    module.__dict__["V9B_CAPTURE_V4_BASELINE_LINEARIZATION"] = capture_baseline
    module.__dict__["V9B_CAPTURE_V6_REJECTED_S2_ACTUAL_DELTA"] = capture_s2
    sys.modules[name] = module
    try:
    ...
    checks = {
        ...
        "callbacks_identity_exact": module.V9B_CAPTURE_V4_BASELINE_LINEARIZATION
        is capture_baseline
        and module.V9B_CAPTURE_V6_REJECTED_S2_ACTUAL_DELTA is capture_s2,
    }
    if not all(checks.values()):
...
def run_isolated_child(kind: str) -> tuple[dict[str, Any], dict[str, Any]]:
    child_code = (
        "import runpy\n"
        f"ns=runpy.run_path({str(SCRIPT)!r},run_name='cw24_v9b_isolated_{kind}')\n"
...
def production_run() -> dict[str, Any]:
    ...
    if not all(runtime_checks.values()):
        raise ProtocolError(f"v9b runtime drift: {runtime_checks}")
    ...
    decision = (
        "GO_CW24_V9B_SPHERICAL_MIDPOINT_LINEAR_PREFLIGHT"
        if preflight_pass
        else "NO_GO_CW24_V9B_SPHERICAL_MIDPOINT_LINEAR_PREFLIGHT"
    )
    result = {
...
def main() -> None:
    ...
    if args.audit_only:
        ...
        if not all(checks.values()):
            raise ProtocolError(f"v9b static audit failed: {checks}")
        print(
            canonical_json(
                {
                    "schema_version": SCHEMA,
        ...
        return
    if OUTPUT.exists():
        raise ProtocolError("frozen CW24 v9b output already exists")
