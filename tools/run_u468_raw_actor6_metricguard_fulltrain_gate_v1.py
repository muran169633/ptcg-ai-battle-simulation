#!/usr/bin/env python3
"""Read-only full-train gate for the selected-row cutting-plane candidate.

The hash-bound cutting-plane v2 runner reconstructs exactly one candidate in
RAM.  Its optional consumer copies that live state to CPU before v2's mandatory
``finally`` restoration.  Only after the selected-row runner has restored raw
does this wrapper reuse the frozen formal-v3 evaluator for all 24,050 train
rows in FLG, PokemonFan, and core5.
...
SCRIPT = TOOLS / "run_u468_raw_actor6_metricguard_fulltrain_gate_v1.py"
SCHEMA = "ptcg-u468-raw-actor6-metricguard-fulltrain-gate-v1"
...
EXPECTED_SELECTED_SUCCESS_ITERATION = 11
EXPECTED_TERMINAL_CUMULATIVE_L2 = 0.006294165313358753
EXPECTED_TERMINAL_CUMULATIVE_SHA256 = (
    "47a001d201db1bba7255ce0b8ccb993cce1649f78f14826782cc0b37e0b31ffc"
)
EXPECTED_FINAL_ACTIVE_PAIR_COUNT = 20
...
def capture_candidate(context: Mapping[str, Any], holder: dict[str, Any]) -> None:
    if holder:
    ...
    actor_audit = helper.verify_actor6_only(
        raw_state, candidate_state, "cutting-plane candidate"
    )
    nonactor_names = sorted(set(raw_state) - set(helper.ACTOR6_NAMES))
    candidate_nonactor_sha = helper.model_state_sha256(
        {name: candidate_state[name] for name in nonactor_names}
    )
    if candidate_nonactor_sha != context["raw_nonactor_sha256"]:
        raise RuntimeError("consumer candidate changed nonactor state")
    ...
    holder.update(
        {
            ...
            "audit": {
                ...
                "RAM_only_CPU_clone_before_v2_finally_restore": True,
            },
        }
    )
...
def run_gate(
    ...
    geometry, geometry_evidence = cutting.import_frozen(
        cutting.GEOMETRY, cutting.GEOMETRY_SHA256, "fulltrain_cutting_geometry_v2"
    )
    ram, ram_evidence = cutting.import_frozen(
        cutting.RAM_RUNNER, cutting.RAM_RUNNER_SHA256, "fulltrain_cutting_ram_v1"
    )
    ...
    def consumer(context: Mapping[str, Any]) -> None:
        capture_candidate(context, captured)
    ...
    if selected_result.get("status") != "selected_row_adaptive_optimization_success":
        if captured:
    ...
    selected_contract_checks = {
        "success_iteration_exact_11": int(selected_decision["success_iteration"])
        ...
        == EXPECTED_TERMINAL_CUMULATIVE_SHA256,
        "final_active_pair_count_exact_20": int(
