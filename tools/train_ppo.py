...
BC_FEATURE_SOURCE_PATH = Path(
    os.environ.get(
        "PTCG_BC_FEATURE_SOURCE",
        str(Path(__file__).resolve().parent / "train_bc_orbit.py"),
    )
).resolve()
if not BC_FEATURE_SOURCE_PATH.is_file():
    raise FileNotFoundError(BC_FEATURE_SOURCE_PATH)
BC_FEATURE_SOURCE_SHA256 = hashlib.sha256(
    BC_FEATURE_SOURCE_PATH.read_bytes()
).hexdigest()
expected_bc_source_sha256 = os.environ.get(
    "PTCG_BC_FEATURE_SOURCE_SHA256"
)
if (
    expected_bc_source_sha256 is not None
    and BC_FEATURE_SOURCE_SHA256 != expected_bc_source_sha256
):
    raise RuntimeError(
        "PTCG_BC_FEATURE_SOURCE_SHA256 mismatch: "
        f"actual={BC_FEATURE_SOURCE_SHA256} "
        f"expected={expected_bc_source_sha256}"
    )
if BC_FEATURE_SOURCE_PATH != (Path(__file__).resolve().parent / "train_bc_orbit.py"):
    ...
    if spec is None or spec.loader is None:
        raise ImportError(
            f"Could not load BC feature source: {BC_FEATURE_SOURCE_PATH}"
        )
    bc_feature_module = importlib.util.module_from_spec(spec)
    sys.modules["train_bc_orbit"] = bc_feature_module
    spec.loader.exec_module(bc_feature_module)
...
class PPOConfig:
    ...
    opponent_meta_weights: dict[str, float]
    opponent_win_rate_window_games: int
    ...
    truncation_as_loss: bool = False
    failed_attempt_as_loss: bool = False
    failed_loss_tail_transitions: int = 32
    champion_gate_interval: int = 0
    champion_gate_games: int = 200
    champion_gate_min_win_rate: float = 0.58
    champion_gate_threshold_inclusive: bool = False
    champion_gate_soft_min_win_rate: float | None = None
    champion_gate_confirmation_games: int = 200
    champion_gate_initial_wait_updates: int = 0
    champion_gate_failure_cooldown_updates: int = 0
    champion_gate_success_cooldown_updates: int = 0
    champion_pool_gate_games: int = 0
    champion_pool_gate_min_win_rate: float = 0.53
    candidate_snapshot_interval: int = 0
    rollback_on_gate_failure: bool = False
    reset_optimizer_on_gate_rollback: bool = False
...
class RunningGame:
    ...
    decisions: int = 0
    # Feed-forward policies may still consume a bounded history of public log
    # deltas.  Keep it per seat because the official engine exposes logs at
    # selection boundaries from that seat's observation stream.
    public_logs: dict[int, list[dict[str, Any]]] = field(
        default_factory=lambda: {0: [], 1: []}
    )


FAILURE_AUDIT_COUNTER_FIELDS = (
    "attempts",
    "attempts_with_learner_transitions",
    "attempts_without_learner_transitions",
    "available_learner_transition_rows",
    "kept_as_loss_attempts",
    "kept_transition_rows",
    "discarded_attempts",
)


class RolloutFailureAudit:
    """Account for every failed rollout attempt without inventing samples."""

    def __init__(self) -> None:
        self.total: Counter[str] = Counter()
        self.by_reason: dict[str, Counter[str]] = defaultdict(Counter)
        self.by_opponent: dict[str, Counter[str]] = defaultdict(Counter)
        self.by_learner_seat: dict[str, Counter[str]] = defaultdict(Counter)
    ...
    def _update(
        ...
        if available_rows < 0 or kept_rows < 0 or kept_rows > available_rows:
            raise ValueError("Invalid failed-attempt transition accounting")
        counter["attempts"] += 1
        counter["available_learner_transition_rows"] += available_rows
        if available_rows:
            counter["attempts_with_learner_transitions"] += 1
        else:
            counter["attempts_without_learner_transitions"] += 1
        if kept_rows:
            counter["kept_as_loss_attempts"] += 1
            counter["kept_transition_rows"] += kept_rows
        else:
            counter["discarded_attempts"] += 1

    @staticmethod
    def _opponent_label(game: RunningGame) -> str:
        return (
            SELFPLAY_OPPONENT_GROUP
            if game.opponent_name is None
            else str(game.opponent_name)
        )

    @staticmethod
    def _learner_seat_label(game: RunningGame) -> str:
        seats = sorted(int(seat) for seat in game.trainable_seats)
        if len(seats) == 1:
            return str(seats[0])
        return "selfplay_both" if seats == [0, 1] else "unknown"
    ...
    def record_game(
        ...
        if not reason:
            raise ValueError("Failed-attempt reason must not be empty")
        for counter in (
            self.total,
            self.by_reason[reason],
            self.by_opponent[self._opponent_label(game)],
            self.by_learner_seat[self._learner_seat_label(game)],
        ):
            self._update(counter, available_rows, kept_rows)

    def record_start_error(
        self,
        requested_opponent: str,
        requested_learner_seat: str,
    ) -> None:
        for counter in (
            self.total,
            self.by_reason["battle_start_error"],
            self.by_opponent[requested_opponent],
            self.by_learner_seat[requested_learner_seat],
        ):
            self._update(counter, available_rows=0, kept_rows=0)

    @staticmethod
    def _render_counter(counter: Counter[str]) -> dict[str, int]:
        return {
            field: int(counter.get(field, 0))
            for field in FAILURE_AUDIT_COUNTER_FIELDS
        }

    def render(
        self,
        started_game_attempts: int,
        valid_games: int,
        battle_start_errors: int,
    ) -> dict[str, Any]:
        failed_started_games = started_game_attempts - valid_games
        expected_hard_failures = failed_started_games + battle_start_errors
        if failed_started_games < 0:
            raise RuntimeError("Valid rollout games exceed started attempts")
        if int(self.total["attempts"]) != expected_hard_failures:
            raise RuntimeError(
                "Failed-attempt audit coverage mismatch: "
                f"audited={self.total['attempts']} "
                f"expected={expected_hard_failures}"
            )
        return {
            "started_game_attempts": int(started_game_attempts),
            "battle_start_error_attempts": int(battle_start_errors),
            "total_attempts_including_start_errors": int(
                started_game_attempts + battle_start_errors
            ),
            "valid_games": int(valid_games),
            "failed_started_games": int(failed_started_games),
            "hard_failure_attempts": int(expected_hard_failures),
            **self._render_counter(self.total),
            "by_reason": {
                name: self._render_counter(counter)
                for name, counter in sorted(self.by_reason.items())
            },
            "by_opponent": {
                name: self._render_counter(counter)
                for name, counter in sorted(self.by_opponent.items())
            },
            "by_learner_seat": {
                name: self._render_counter(counter)
                for name, counter in sorted(self.by_learner_seat.items())
            },
        }
...
def row_weighted_minibatch_mean(
    batch_means_and_rows: list[tuple[float, int]],
) -> float:
    """Reconstruct a row mean from variable-sized minibatch means."""
    if not batch_means_and_rows:
        raise ValueError("Cannot average an empty minibatch sequence")
    if any(rows <= 0 for _, rows in batch_means_and_rows):
        raise ValueError("Minibatch row counts must be positive")
    total_rows = sum(rows for _, rows in batch_means_and_rows)
    return sum(mean * rows for mean, rows in batch_means_and_rows) / total_rows
...
def kept_episode_count(transitions: list[dict[str, Any]]) -> int:
    """Count distinct kept rollout games with strict game_uid validation."""
    if not transitions:
        raise ValueError("Cannot count episodes in an empty rollout")
    ...
    for row_index, transition in enumerate(transitions):
        ...
        if isinstance(game_uid, bool) or not isinstance(game_uid, int):
            raise ValueError(
                "Episode counting requires integer game_uid values; "
                f"row {row_index} has {game_uid!r}"
            )
        game_uids.add(game_uid)
    if missing_rows:
        raise ValueError(
            "Episode counting requires game_uid on every transition; "
            f"missing rows={missing_rows[:16]}"
        )
    return len(game_uids)
...
class OpponentQuotaController:
    ...
    def __init__(
        ...
        *,
        ...
        win_rate_window_games: int = 200,
        inverse_min_factor: float = 0.5,
        inverse_max_factor: float = 2.5,
    ) -> None:
        if mode not in {"fixed", "adaptive", "meta_inverse_window"}:
        ...
        self.caps = {
            name: int(
                caps.get(
                    ...
                    (
                        games_per_update
                        if mode == "meta_inverse_window"
                        else self.base_quotas[name]
                    ),
                )
            )
            for name in self.opponent_names
        ...
        self.seed = int(seed)
        self.meta_weights = {
            name: float((meta_weights or {}).get(name, 0.0))
            for name in self.opponent_names
        }
        self.win_rate_window_games = int(win_rate_window_games)
        self.inverse_min_factor = float(inverse_min_factor)
        self.inverse_max_factor = float(inverse_max_factor)
        self.outcome_window: dict[str, list[float]] = {
            name: [] for name in self.opponent_names
        }
        ...
        self.current_meta_window_audit: dict[str, dict[str, float | int]] | None = None
    ...
    def _validate(self) -> None:
        ...
        if self.mode == "meta_inverse_window":
            if (
                set(self.meta_weights) != set(self.opponent_names)
                or any(
                    not math.isfinite(weight) or weight <= 0.0
                    for weight in self.meta_weights.values()
                )
            ):
                raise ValueError(
                    "meta_inverse_window requires one positive finite meta "
                    "weight per opponent"
                )
            if self.win_rate_window_games < 1:
                raise ValueError("Win-rate window size must be positive")
            if (
                not math.isfinite(self.inverse_min_factor)
                or not math.isfinite(self.inverse_max_factor)
                or self.inverse_min_factor <= 0.0
                or self.inverse_max_factor <= 0.0
                or self.inverse_min_factor > self.inverse_max_factor
            ):
                raise ValueError("Invalid inverse win-rate factor bounds")
    ...
    def meta_window_audit(self) -> dict[str, dict[str, float | int]]:
        """Return multiplicative recent-meta and inverse-win-rate weights."""

        result: dict[str, dict[str, float | int]] = {}
        for name in self.opponent_names:
            values = self.outcome_window[name]
            games = len(values)
            wins_equivalent = float(sum(values))
            # Beta(2, 2) prevents an unseen or tiny-sample matchup from
            # receiving an unbounded inverse weight.
            smoothed_win_rate = (2.0 + wins_equivalent) / (4.0 + games)
            raw_factor = 0.5 / max(smoothed_win_rate, 1e-12)
            factor = min(
                max(raw_factor, self.inverse_min_factor),
                self.inverse_max_factor,
            )
            result[name] = {
                "window_games": games,
                "window_wins_equivalent": wins_equivalent,
                "smoothed_win_rate": smoothed_win_rate,
                "raw_inverse_factor": raw_factor,
                "clipped_inverse_factor": factor,
                "meta_weight": self.meta_weights[name],
                "combined_weight": self.meta_weights[name] * factor,
            }
        return result
    ...
    def begin_update(
        ...
        if recomputed:
            meta_window: dict[str, dict[str, float | int]] | None = None
            if self.mode == "fixed":
                dynamic = {name: 0 for name in self.opponent_names}
            else:
                if self.mode == "meta_inverse_window":
                    meta_window = self.meta_window_audit()
                    allocation_weights = [
                        float(meta_window[name]["combined_weight"])
                        for name in self.opponent_names
                    ]
                else:
                    meta_window = None
            ...
            self.current_meta_window_audit = copy.deepcopy(meta_window)
        ...
        audit = {
            ...
            "allocation_model": (
                {
                    "base": "recent_day_exact_deck_seat_frequency",
                    "adjustment": "clipped_0.5_over_beta22_smoothed_win_rate",
                    "window_games_per_opponent": self.win_rate_window_games,
                    "inverse_min_factor": self.inverse_min_factor,
                    "inverse_max_factor": self.inverse_max_factor,
                    "meta_window": copy.deepcopy(
                        self.current_meta_window_audit
                    ),
                }
                if self.mode == "meta_inverse_window"
                else {
                    "base": "minimum_quota",
                    "adjustment": "beta_failure_ucb",
                }
            ),
    ...
    def finish_update(
        ...
        outcome_sequences: dict[str, list[str]] | None = None,
    ) -> dict[str, Any]:
        if sum(
        ...
        validated_outcome_sequences: dict[str, list[str]] = {}
        if self.mode == "meta_inverse_window":
            for name in self.opponent_names:
                sequence = list((outcome_sequences or {}).get(name, []))
                if Counter(sequence) != Counter(
                    {
                        "win": actual_outcomes[name]["wins"],
                        "loss": actual_outcomes[name]["losses"],
                        "draw": actual_outcomes[name]["draws"],
                    }
                ):
                    raise RuntimeError(
                        f"Update {update} rolling outcome sequence mismatch "
                        f"for {name}"
                    )
                validated_outcome_sequences[name] = sequence

        # Mutate posterior/window state only after every aggregate, seat, and
        # rolling-sequence invariant has passed, so a rejected update cannot
        # be half-applied.
        for name in self.opponent_names:
            for key in ("wins", "losses", "draws"):
            ...
            if self.mode == "meta_inverse_window":
                ...
                self.outcome_window[name].extend(
                    numeric[value]
                    for value in validated_outcome_sequences[name]
                )
                self.outcome_window[name] = self.outcome_window[name][
                    -self.win_rate_window_games :
                ]
        ...
        if self.mode == "meta_inverse_window":
            audit["meta_window_after"] = self.meta_window_audit()
        return audit
    ...
    def state_dict(self) -> dict[str, Any]:
        return {
            ...
            "win_rate_window_games": self.win_rate_window_games,
            "inverse_min_factor": self.inverse_min_factor,
            "inverse_max_factor": self.inverse_max_factor,
            "outcome_window": copy.deepcopy(self.outcome_window),
            ...
            "current_meta_window_audit": copy.deepcopy(
                self.current_meta_window_audit
            ),
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        ...
        saved_identity = {
            key: (
                ...
                if key in backward_compatible_keys
                else state.get(key)
            )
            for key in identity
        }
        ...
        for audit in self.observed.values():
            beta_failure_posterior(audit)
        raw_window = state.get("outcome_window")
        if not isinstance(raw_window, dict):
            if self.mode == "meta_inverse_window":
                raise ValueError("Resume checkpoint lacks outcome windows")
            raw_window = {}
        self.outcome_window = {
            name: [float(value) for value in raw_window.get(name, [])]
            for name in self.opponent_names
        }
        if any(
            len(values) > self.win_rate_window_games
            or any(value not in {0.0, 0.5, 1.0} for value in values)
            for values in self.outcome_window.values()
        ):
            raise ValueError("Resume checkpoint has invalid outcome windows")
        ...
        raw_meta_window_audit = state.get("current_meta_window_audit")
        self.current_meta_window_audit = (
            copy.deepcopy(raw_meta_window_audit)
            if isinstance(raw_meta_window_audit, dict)
            else None
        )
...
def live_feature(
    ...
    deck_hash: str,
    persistent_logs: list[dict[str, Any]] | None = None,
) -> dict[str, Any] | None:
    ...
    row = {
        ...
        "terminal_reward": 0.0,
        "sample_weight": 1.0,
        "_persistent_logs": list(persistent_logs or []),
    }
...
def collate_features(
    ...
    batch = collate_live_features_fast_cpu(features, model_config)
    if batch is None:
...
def collate_live_features_fast_cpu(
    features: list[dict[str, Any]],
    model_config: dict[str, Any],
) -> dict[str, torch.Tensor] | None:
    """Vectorized CPU collation for action-free live rollout features.

    The training collator supports arbitrary demonstration targets and ragged
    categorical field widths.  Live inference rows are simpler: they have no
    selected action and featurization emits fixed-width entity/option fields.
    Filling one NumPy slice per row avoids thousands of tiny ``torch.tensor``
    constructions in every rollout inference round.  Unexpected row shapes
    return ``None`` and retain the fully general legacy path.
    """

    if not features or any(
        int(row.get("action_count", 0)) != 0
        or bool(row.get("action_sequence"))
        or "policy_team_weight" in row
        for row in features
    ):
        return None
    ...
    max_options = max(len(row["targets"]) for row in features)

    global_fields = np.zeros((batch_size, entity_fields), dtype=np.int64)
    global_field_mask = np.zeros((batch_size, entity_fields), dtype=np.bool_)
    global_numeric = np.zeros(
        (batch_size, GLOBAL_NUMERIC_SIZE), dtype=np.float32
    )
    state_fields = np.zeros(
        (batch_size, max_state_entities, entity_fields), dtype=np.int64
    )
    state_field_mask = np.zeros_like(state_fields, dtype=np.bool_)
    state_numeric = np.zeros(
        (batch_size, max_state_entities, ENTITY_NUMERIC_SIZE),
        dtype=np.float32,
    )
    state_mask = np.zeros(
        (batch_size, max_state_entities), dtype=np.bool_
    )
    option_field_values = np.zeros(
        (batch_size, max_options, option_fields), dtype=np.int64
    )
    option_field_mask = np.zeros_like(option_field_values, dtype=np.bool_)
    option_numeric = np.zeros(
        (batch_size, max_options, OPTION_NUMERIC_SIZE), dtype=np.float32
    )
    option_mask = np.zeros((batch_size, max_options), dtype=np.bool_)
    targets = np.zeros((batch_size, max_options), dtype=np.float32)
    min_counts = np.zeros(batch_size, dtype=np.int64)
    max_counts = np.zeros(batch_size, dtype=np.int64)
    contexts = np.zeros(batch_size, dtype=np.int64)
    sample_weights = np.ones(batch_size, dtype=np.float32)
    win_targets = np.zeros(batch_size, dtype=np.float32)

    try:
        for batch_index, row in enumerate(features):
            gf = np.asarray(row["global_fields"], dtype=np.int64)
            gf_width = min(gf.shape[0], entity_fields)
            global_fields[batch_index, :gf_width] = gf[:gf_width]
            global_field_mask[batch_index, :gf_width] = True
            global_numeric[batch_index] = np.asarray(
                row["global_numeric"], dtype=np.float32
            )

            entity_count = min(
                len(row["entity_fields"]), max_state_entities
            )
            if entity_count:
                en = np.asarray(
                    row["entity_numeric"][:entity_count], dtype=np.float32
                )
                if en.ndim != 2:
                    return None
                raw_entity_fields = row["entity_fields"][:entity_count]
                try:
                    ef = np.asarray(raw_entity_fields, dtype=np.int64)
                except ValueError:
                    ef = np.empty(0, dtype=np.int64)
                if ef.ndim == 2:
                    ef_width = min(ef.shape[1], entity_fields)
                    state_fields[
                        batch_index, :entity_count, :ef_width
                    ] = ef[:, :ef_width]
                    state_field_mask[
                        batch_index, :entity_count, :ef_width
                    ] = True
                else:
                    for entity_index, fields in enumerate(raw_entity_fields):
                        field_values = np.asarray(fields, dtype=np.int64)
                        ef_width = min(field_values.shape[0], entity_fields)
                        state_fields[
                            batch_index, entity_index, :ef_width
                        ] = field_values[:ef_width]
                        state_field_mask[
                            batch_index, entity_index, :ef_width
                        ] = True
                state_numeric[batch_index, :entity_count] = en
                state_mask[batch_index, :entity_count] = True

            option_count = len(row["targets"])
            if option_count:
                on = np.asarray(row["option_numeric"], dtype=np.float32)
                if on.ndim != 2:
                    return None
                raw_option_fields = row["option_fields"]
                try:
                    of = np.asarray(raw_option_fields, dtype=np.int64)
                except ValueError:
                    of = np.empty(0, dtype=np.int64)
                if of.ndim == 2:
                    of_width = min(of.shape[1], option_fields)
                    option_field_values[
                        batch_index, :option_count, :of_width
                    ] = of[:, :of_width]
                    option_field_mask[
                        batch_index, :option_count, :of_width
                    ] = True
                else:
                    for option_index, fields in enumerate(raw_option_fields):
                        field_values = np.asarray(fields, dtype=np.int64)
                        of_width = min(field_values.shape[0], option_fields)
                        option_field_values[
                            batch_index, option_index, :of_width
                        ] = field_values[:of_width]
                        option_field_mask[
                            batch_index, option_index, :of_width
                        ] = True
                option_numeric[batch_index, :option_count] = on
                option_mask[batch_index, :option_count] = True
                targets[batch_index, :option_count] = np.asarray(
                    row["targets"], dtype=np.float32
                )
            min_counts[batch_index] = int(row["min_count"])
            max_counts[batch_index] = int(row["max_count"])
            contexts[batch_index] = int(row["context"])
            sample_weights[batch_index] = float(row["sample_weight"])
            win_targets[batch_index] = float(row["win_target"])
    except (KeyError, TypeError, ValueError):
        return None

    action_counts = np.zeros(batch_size, dtype=np.int64)
    action_sequences = np.full(
        (batch_size, BC_MAX_ACTION_COUNT), -1, dtype=np.int64
    )
    arrays = {
        ...
        "option_fields": option_field_values,
...
class OpponentSamplingReweighter:
    """Recent-meta base probabilities with inverse rolling-win adjustment."""
    ...
    def __init__(
        ...
        *,
        ...
        if len(opponent_names) != len(set(opponent_names)):
            raise ValueError("Sampling reweight opponent names must be unique")
        if set(meta_weights) != set(opponent_names):
            raise ValueError(
                "Sampling meta weights must exactly cover permanent opponents"
            )
        if any(
            not math.isfinite(weight) or weight <= 0.0
            for weight in meta_weights.values()
        ):
            raise ValueError("Sampling meta weights must be finite and positive")
        if window_games < 1:
            raise ValueError("Sampling win-rate window must be positive")
        if (
            ...
            raise ValueError("Invalid sampling inverse factor bounds")
        self.opponent_names = list(opponent_names)
        self.meta_weights = {
            name: float(meta_weights[name]) for name in self.opponent_names
        }
    ...
    def audit(self) -> dict[str, dict[str, float | int]]:
        ...
        for name in self.opponent_names:
            values = self.outcome_window[name]
            smoothed_win_rate = (2.0 + sum(values)) / (4.0 + len(values))
            raw_factor = 0.5 / max(smoothed_win_rate, 1e-12)
            factor = min(
                max(raw_factor, self.inverse_min_factor),
                self.inverse_max_factor,
            ...
            rows[name] = {
                "window_games": len(values),
                "window_wins_equivalent": float(sum(values)),
                "smoothed_win_rate": float(smoothed_win_rate),
                "raw_inverse_factor": float(raw_factor),
                "clipped_inverse_factor": float(factor),
                "meta_weight": self.meta_weights[name],
                "combined_weight": self.meta_weights[name] * factor,
            }
        total = sum(float(row["combined_weight"]) for row in rows.values())
        for row in rows.values():
            row["sampling_probability_given_frozen_opponent"] = (
                float(row["combined_weight"]) / total
            )
        return rows
    ...
    def sampling_weights(
        ...
        weights = [
            (
                float(audit[opponent.name]["combined_weight"])
        ...
        if sum(weights) <= 0.0:
            raise RuntimeError("Rolling opponent weights sum to zero")
        return weights

    def observe(self, outcome_sequences: Mapping[str, list[str]]) -> None:
        unknown = set(outcome_sequences) - set(self.opponent_names)
        if unknown:
            raise ValueError(
                "Rolling outcomes contain unknown opponents: "
                + ", ".join(sorted(unknown))
            )
        numeric = {"win": 1.0, "draw": 0.5, "loss": 0.0}
        for name in self.opponent_names:
            sequence = list(outcome_sequences.get(name, []))
            if any(value not in numeric for value in sequence):
                raise ValueError(f"Invalid rolling outcome for {name}")
            self.outcome_window[name].extend(
                numeric[value] for value in sequence
            )
            self.outcome_window[name] = self.outcome_window[name][
                -self.window_games :
            ]
    ...
    def load_state_dict(self, state: Mapping[str, Any]) -> None:
        ...
        if {key: state.get(key) for key in identity} != identity:
            raise ValueError(
                "Resume checkpoint opponent sampling configuration mismatch"
            )
        raw = state.get("outcome_window")
        if not isinstance(raw, Mapping):
            raise ValueError("Resume checkpoint lacks sampling outcome windows")
        windows = {
            name: [float(value) for value in raw.get(name, [])]
            for name in self.opponent_names
        }
        if any(
            len(values) > self.window_games
            or any(value not in {0.0, 0.5, 1.0} for value in values)
            for values in windows.values()
        ):
            raise ValueError("Resume checkpoint has invalid sampling windows")
        self.outcome_window = windows
        self.resume_state_loaded = True
...
def build_opponent_quota_controller(
    ...
    configured_name_sets = {
        ...
        "meta weight": set(getattr(config, "opponent_meta_weights", {})),
    }
    ...
    if config.opponent_quota_mode == "meta_inverse_window":
        missing_meta_weights = [
            name for name in permanent_names
            if name not in config.opponent_meta_weights
        ]
        if missing_meta_weights:
            raise ValueError(
                "Meta opponent weights must cover every permanent opponent: "
                + ", ".join(missing_meta_weights)
            )
    controller = OpponentQuotaController(
        ...
        seed=config.seed,
        meta_weights=getattr(config, "opponent_meta_weights", {}),
        win_rate_window_games=getattr(
            config,
            "opponent_win_rate_window_games",
            200,
        ),
        inverse_min_factor=getattr(
            config,
            "opponent_inverse_min_factor",
            0.5,
        ),
        inverse_max_factor=getattr(
            config,
            "opponent_inverse_max_factor",
            2.5,
        ),
    )
...
def validate_saved_opponent_quota_state(state: dict[str, Any]) -> None:
    ...
    controller = OpponentQuotaController(
        ...
        meta_weights=dict(state.get("meta_weights") or {}),
        win_rate_window_games=int(
            state.get("win_rate_window_games", 200)
        ),
        inverse_min_factor=float(state.get("inverse_min_factor", 0.5)),
        inverse_max_factor=float(state.get("inverse_max_factor", 2.5)),
    )
...
def validate_opponent_quota_resume_transition(
    *,
    ...
    if (
        current_mode in {"fixed", "adaptive", "meta_inverse_window"}
        and checkpoint_mode in {"fixed", "adaptive", "meta_inverse_window"}
        and checkpoint_state is None
    ):
        raise ValueError(
    ...
    if (
        ...
        raise ValueError(
            "Starting legacy random opponent sampling from an exact-quota "
            "checkpoint requires --reset-optimizer-on-resume"
        )
...
def learner_transition_count(game: RunningGame) -> int:
    """Return the number of learner rows already collected for one game."""
    indices = [
        index
        for seat_indices in game.transition_indices.values()
        for index in seat_indices
    ]
    if len(indices) != len(set(indices)):
        raise RuntimeError("A rollout transition belongs to multiple learner seats")
    return len(indices)


def finalize_failed_episode_tail(
    game: RunningGame,
    transitions: list[dict[str, Any]],
    gamma: float,
    gae_lambda: float,
    ...
    """Keep only the final learner rows with terminal outcome zero.

    The tail budget applies once across the game, not once per seat.  This
    prevents a self-play failure from receiving twice the configured budget.
    A failure before any learner decision returns zero and creates no sample.
    """
    if tail_transitions < 1:
        raise ValueError("Failed-attempt tail transition count must be positive")
    ...
    for index in all_indices:
        transitions[index]["keep"] = False
    selected_indices = set(all_indices[-tail_transitions:])
    if not selected_indices:
        return 0

    tail_game = copy.copy(game)
    tail_game.transition_indices = {
        seat: [index for index in indices if index in selected_indices]
        for seat, indices in game.transition_indices.items()
    }
    for index in selected_indices:
        transitions[index]["keep"] = True
    finalize_episode(
        tail_game,
        transitions,
        result=2,
    ...
    for index in selected_indices:
        transitions[index]["failed_attempt"] = True
        transitions[index]["failure_reason"] = failure_reason
    return len(selected_indices)


def finalize_truncated_episode(
    game: RunningGame,
    transitions: list[dict[str, Any]],
    gamma: float,
    gae_lambda: float,
    ...
    """Finalize a decision-limit trajectory and return kept transition count.

    A truncation has no winner, so ``result=2`` gives every trainable seat the
    existing zero terminal outcome.  The legacy behavior excludes the entire
    trajectory.  ``as_loss=True`` keeps it with return zero, allowing PPO to
    learn that policies which reach the decision limit are undesirable while
    preserving the public win=1/loss-or-draw=0 reward scale.
    """
    transition_count = sum(
        len(indices) for indices in game.transition_indices.values()
    )
    finalize_episode(
        game,
        transitions,
    ...
    return transition_count if as_loss else 0


def validate_failed_attempt_loss_configuration(
    failed_attempt_as_loss: bool,
    failed_loss_tail_transitions: int,
    truncation_as_loss: bool,
) -> None:
    if failed_loss_tail_transitions < 1:
        raise ValueError("--failed-loss-tail-transitions must be positive")
    if failed_attempt_as_loss and truncation_as_loss:
        raise ValueError(
            "--failed-attempt-as-loss is mutually exclusive with the legacy "
            "--truncation-as-loss mode"
        )


def reward_manifest(config: PPOConfig) -> dict[str, Any]:
    reward: dict[str, Any] = {
        "win": 1.0,
        "loss": 0.0,
        "draw": 0.0,
        "truncation_handling": (
            "loss_return_0"
            if config.truncation_as_loss
            else "exclude_and_replace"
        ),
    }
    if config.failed_attempt_as_loss:
        reward["truncation_handling"] = "tail_terminal_outcome_0_and_replace"
        reward["failed_attempt_handling"] = {
            "enabled": True,
            "reasons": [
                "invalid_observation",
                "battle_step_error",
            ...
            "outcome": 0.0,
            "tail_transitions": config.failed_loss_tail_transitions,
            "tail_scope": "last_learner_transitions_per_game",
            "no_learner_transition": "audit_only_no_sample",
            "battle_start_error": "audit_only_no_sample",
            "valid_game_quota": "replace_same_claim_when_exact_quota",
        }
    return reward
...
def collect_rollout(
    ...
    opponent_sampling_reweighter: OpponentSamplingReweighter | None = None,
    ...
    league_outcome_sequence: dict[str, list[str]] = defaultdict(list)
    started = time.time()
    ...
    opponent_sampling_weights = (
        ...
        else resolve_opponent_sampling_weights(
            opponents,
    ...
    planned_seat_quotas: dict[str, dict[str, int]] | None = None
    failure_audit = RolloutFailureAudit()

    def record_start_error(
        requested_opponent_index: int | None,
        requested_learner_seat: int | None,
    ) -> None:
        stats["start_errors"] += 1
        if not opponents:
            opponent_label = SELFPLAY_OPPONENT_GROUP
            learner_seat_label = "selfplay_both"
        else:
            opponent_label = (
                opponents[requested_opponent_index].name
                if (
                    config.league_probability == 1.0
                    and requested_opponent_index is not None
                )
                else "unknown"
            )
            learner_seat_label = (
                str(requested_learner_seat)
                if requested_learner_seat is not None
                else "unknown"
            )
    ...
    def finalize_failed_attempt(
        ...
        *,
        ...
        available_rows = learner_transition_count(game)
        if config.failed_attempt_as_loss:
            kept_rows = finalize_failed_episode_tail(
                game,
                transitions,
                gamma=config.gamma,
        ...
        elif legacy_truncation:
            kept_rows = finalize_truncated_episode(
                game,
                transitions,
                gamma=config.gamma,
                gae_lambda=config.gae_lambda,
                as_loss=config.truncation_as_loss,
            )
        else:
            finalize_episode(
                game,
        ...
        if kept_rows:
            stats["failed_attempts_kept_as_loss"] += 1
            stats["failed_transitions_kept"] += kept_rows
            stats[f"{reason}_attempts_kept_as_loss"] += 1
            stats[f"{reason}_transitions_kept"] += kept_rows
        return available_rows, kept_rows
    ...
    attempts = 0
    while games:
        # Frozen opponents may intentionally share one immutable model across
        # many deck bindings.  Batch those decisions by model identity instead
        # of issuing one small forward pass per deck/opponent index.
        groups: dict[
            tuple[str, int],
            list[tuple[RunningGame, dict[str, Any], int]],
        ] = defaultdict(list)
        ...
        for game in games:
            ...
            groups[policy_key].append((game, feature, policy_index))
        ...
        for _, items in groups.items():
            policy_index = items[0][2]
            if any(item[2] == -1 for item in items) != (policy_index == -1):
                raise RuntimeError("Learner/frozen rollout batch was mixed")
            if policy_index != -1:
                shared_model = opponents[policy_index].model
                if any(
                    opponents[item[2]].model is not shared_model
                    or opponents[item[2]].canonical_order
                    != opponents[policy_index].canonical_order
                    for item in items
                ):
                    raise RuntimeError(
                        "Shared rollout batch has inconsistent frozen policy"
                    )
            features = [feature for _, feature, _ in items]
            ...
            for item_index, (game, feature, item_policy_index) in enumerate(items):
                if item_policy_index != -1:
                    policy_index = item_policy_index
        ...
        for game in list(games):
            ...
            if result != -1:
                ...
                if len(game.trainable_seats) == 1:
                    ...
                    if result == current_seat:
                        ...
                        league_outcome_sequence[opponent_name].append("win")
                    elif result == 2:
                        ...
                        league_outcome_sequence[opponent_name].append("draw")
                    else:
                        ...
                        league_outcome_sequence[opponent_name].append("loss")
                else:
            ...
            if game.decisions >= config.max_game_decisions:
                ...
                _, kept_truncated_transitions = finalize_failed_attempt(
                    game,
                    "max_game_decisions",
                    legacy_truncation=True,
                )
                if config.truncation_as_loss or kept_truncated_transitions:
                    stats["truncated_games_kept_as_loss"] += 1
                    stats["truncated_transitions_kept"] += (
                        kept_truncated_transitions
                    )
                finished.append(game)
    ...
    failure_metrics = failure_audit.render(
        started_game_attempts=attempts,
    ...
    stats["kept_episode_count"] = kept_episode_count(kept)
    stats["attempted_games"] = attempts
    stats["attempted_games_including_start_errors"] = (
        attempts + stats["start_errors"]
    )
    stats["failed_started_games"] = attempts - valid_completed
    stats["hard_failure_attempts"] = failure_metrics[
    ...
    result_stats["failure_audit"] = failure_metrics
    ...
    if opponent_quota_controller is not None:
        ...
        quota_audit = opponent_quota_controller.finish_update(
            ...
            outcome_sequences={
                name: list(values)
                for name, values in league_outcome_sequence.items()
            },
        )
    ...
    if opponent_sampling_reweighter is not None:
        result_stats["opponent_sampling_reweight_before"] = (
            opponent_sampling_reweighter.audit()
        )
        # Retain the ordered outcomes so a distributed wrapper can merge every
        # rank into one global rolling window before the next PPO update.
        result_stats["opponent_outcome_sequences"] = {
            name: list(values)
            for name, values in league_outcome_sequence.items()
        }
        opponent_sampling_reweighter.observe(
            {
                name: list(values)
                for name, values in league_outcome_sequence.items()
            }
        )
        result_stats["opponent_sampling_reweight_after"] = (
            opponent_sampling_reweighter.audit()
        )
...
def ppo_update_constrained(
    ...
    for epoch in range(config.ppo_epochs):
        ...
        epoch_approx_kl: list[tuple[float, int]] = []
        ...
        for start in range(0, len(transitions), config.minibatch_size):
            ...
            epoch_approx_kl.append((float(approx_kl.detach()), batch_rows))
        ...
        aggregate_kl_exceeded = (
            epoch_approx_kl
            and row_weighted_minibatch_mean(epoch_approx_kl)
...
def audit_actor_reduction_gradients(
    ...
    candidate_multipliers, weight_audit = actor_reduction_multipliers(
        ...
        expected_episode_count=(
            kept_episode_count(transitions)
            if candidate_reduction == "episode_mean"
            else None
        ),
    )
...
def evaluate_head_to_head(
    ...
    stats: Counter[str] = Counter()
    games: list[
        tuple[RawBattle, int, int, dict[int, list[dict[str, Any]]]]
    ] = []
    ...
    def start_evaluation_game() -> tuple[
        RawBattle,
        int,
        int,
        ...
        return (
            RawBattle(seat_decks[0], seat_decks[1]),
            learner_seat,
            0,
            {0: [], 1: []},
        )
...
def champion_gate_promotes(
    evaluation: dict[str, Any],
    minimum_win_rate: float,
    *,
    inclusive: bool = False,
) -> bool:
    """Return whether a candidate clears the requested champion threshold."""
    valid_games = int(evaluation.get("valid_games", 0))
    win_rate = float(evaluation.get("win_rate", -1.0))
    if valid_games <= 0:
        return False
    if inclusive:
...
def combine_head_to_head_evaluations(
    evaluations: list[dict[str, Any]],
) -> dict[str, Any]:
    """Combine disjoint head-to-head panels without averaging percentages."""
    valid_games = sum(int(row.get("valid_games", 0)) for row in evaluations)
    wins = sum(int(row.get("wins", 0)) for row in evaluations)
    losses = sum(int(row.get("losses", 0)) for row in evaluations)
    draws = sum(int(row.get("draws", 0)) for row in evaluations)
    invalid_games = sum(
        int(row.get("invalid_games", 0)) for row in evaluations
    )
    decisions = sum(
        float(row.get("mean_decisions", 0.0))
        * int(row.get("valid_games", 0))
    ...
    return {
        ...
        "mean_decisions": decisions / max(valid_games, 1),
        "max_active_games": max(
            (int(row.get("max_active_games", 0)) for row in evaluations),
            default=0,
        ),
        "seconds": sum(float(row.get("seconds", 0.0)) for row in evaluations),
    }


def allocate_even_weighted_games(
    weights: list[float],
    games: int,
) -> list[int]:
    """Largest-remainder allocation in balanced-seat game pairs."""

    if games < 2 or games % 2:
        raise ValueError("Weighted pool games must be positive and even")
    if not weights or any(not math.isfinite(value) or value < 0 for value in weights):
        raise ValueError("Weighted pool probabilities are invalid")
    total = sum(weights)
    if total <= 0.0:
        raise ValueError("Weighted pool probabilities sum to zero")
    pair_target = games // 2
    raw_pairs = [pair_target * value / total for value in weights]
    pair_counts = [math.floor(value) for value in raw_pairs]
    remaining = pair_target - sum(pair_counts)
    order = sorted(
        range(len(weights)),
        key=lambda index: (raw_pairs[index] - pair_counts[index], weights[index]),
        reverse=True,
    )
    for index in order[:remaining]:
        pair_counts[index] += 1
    allocations = [2 * value for value in pair_counts]
    if sum(allocations) != games:
        raise RuntimeError("Weighted pool allocation does not preserve games")
    return allocations


def evaluate_weighted_opponent_pool(
    current_model: EntityOptionPolicy,
    opponents: list[FrozenOpponent],
    ...
    """Evaluate one aggregate panel using the live league distribution."""

    allocations = allocate_even_weighted_games(sampling_weights, games)
    panels: list[dict[str, Any]] = []
    by_opponent: dict[str, dict[str, Any]] = {}
    for opponent, weight, opponent_games in zip(
        opponents,
        sampling_weights,
        allocations,
    ):
        ...
        evaluation = evaluate_head_to_head(
            ...
            device,
            opponent_games,
            min(environments, opponent_games),
            max_game_decisions,
            current_canonical_order=False,
            opponent_canonical_order=opponent.canonical_order,
            opponent_deck=opponent.deck,
        )
        panels.append(evaluation)
        by_opponent[opponent.name] = {
            **evaluation,
            "allocated_games": opponent_games,
            "sampling_weight": float(weight),
        }
    combined = combine_head_to_head_evaluations(panels)
    combined.update(
        {
            "requested_games": games,
            "allocation_method": "largest_remainder_balanced_seat_pairs",
...
def save_ppo_checkpoint(
    ...
    payload = {
        ...
        "opponent_sampling_reweight_state": (
            opponent_sampling_reweighter.state_dict()
            if opponent_sampling_reweighter is not None
            else None
        ),
        "reward": reward_manifest(config),
        "value_trunk_gradient": value_trunk_gradient_audit(
...
def save_candidate_snapshot(
    ...
    """Persist lightweight policy weights so transient candidates remain testable."""
    ...
    payload = {
        ...
        "config": asdict(config),
        "model_config": model_config,
        "learner_deck_hash": compute_deck_hash(read_deck(Path(config.deck))),
        "model_state_dict": model.state_dict(),
        "update": update,
        "metrics": metrics,
        "snapshot_kind": "candidate_policy_only",
    }
...
def rollback_to_champion(
    current_model: EntityOptionPolicy,
    champion_model: EntityOptionPolicy,
    optimizer: torch.optim.Optimizer,
    replay_optimizer: torch.optim.Optimizer | None,
    *,
    reset_optimizer: bool,
) -> dict[str, Any]:
    """Restore the accepted policy after a rejected gate candidate."""

    current_model.load_state_dict(champion_model.state_dict(), strict=True)
    optimizer_reset = False
    replay_optimizer_reset = False
    if reset_optimizer:
        optimizer.state.clear()
        optimizer_reset = True
        if replay_optimizer is not None:
            replay_optimizer.state.clear()
            replay_optimizer_reset = True
    return {
        "applied": True,
        "optimizer_reset": optimizer_reset,
        "replay_optimizer_reset": replay_optimizer_reset,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--opponent-quota-mode",
        choices=("legacy", "fixed", "adaptive", "meta_inverse_window"),
        default="legacy",
        help=(
            "Use legacy random sampling (default), a fixed exact per-update "
            "quota, an uncertainty-adaptive exact quota, or an exact recent-"
            "meta distribution multiplied by inverse rolling win rate."
        ),
    )
    parser.add_argument(
        "--opponent-base-quota",
    ...
    parser.add_argument(
        "--opponent-meta-weight",
        action="append",
        nargs=2,
        metavar=("NAME", "WEIGHT"),
        default=[],
        help=(
            "Recent-arena base frequency for one permanent opponent in "
            "legacy rolling-sampling or meta_inverse_window exact-quota "
            "mode; repeat for every opponent."
        ),
    )
    parser.add_argument(
        ...
        default=200,
        help=(
            "Per-opponent rolling game window used by meta_inverse_window "
            "quota mode (default: 200)."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Lower clip for inverse rolling-win-rate adjustment.",
    )
    parser.add_argument(
        "--opponent-inverse-max-factor",
        type=float,
        default=2.5,
        help="Upper clip for inverse rolling-win-rate adjustment.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Challenge the current best policy every N updates; zero keeps "
            "the legacy evaluation-score best-checkpoint behavior."
        ),
    )
    parser.add_argument(
        "--champion-gate-games",
        ...
        metavar="GAMES",
        help=(
            "Balanced-seat same-deck games in each candidate-versus-champion "
            "promotion challenge (default: 200)."
        ),
    )
    parser.add_argument(
        ...
        metavar="RATE",
        help=(
            "Strict candidate win-rate threshold for replacing best.pt "
            "(default: 0.58; equality does not promote)."
        ),
    )
    parser.add_argument(
        ...
        metavar="RATE",
        help=(
            "Optional inclusive soft threshold. A first panel at or above "
            "this rate but below the hard threshold receives an independent "
            "confirmation panel; the combined rate must remain at or above "
            "the soft threshold. Setting this also makes the hard threshold "
            "inclusive."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Promote when the candidate equals the hard threshold. The "
            "legacy default requires it to be strictly greater."
        ),
    )
    parser.add_argument(
        ...
        metavar="GAMES",
        help=(
            "Balanced-seat games in the independent soft-gate confirmation "
            "panel (default: 200)."
        ),
    )
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Updates before the first champion challenge. Zero reuses "
            "--champion-gate-interval."
        ),
    )
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Updates to wait after a failed challenge. Zero reuses "
            "--champion-gate-interval."
        ),
    )
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Updates to wait after a promotion. Use a longer value to keep "
            "historical league generations separated. Zero reuses "
            "--champion-gate-interval."
        ),
    )
    parser.add_argument(
        ...
        metavar="GAMES",
        help=(
            "After the mirror gate passes, play this many aggregate balanced-"
            "seat games against the weighted opponent pool. Zero disables "
            "the second-stage gate."
        ),
    )
    parser.add_argument(
        ...
        metavar="RATE",
        help=(
            "Strict aggregate opponent-pool win-rate threshold for the "
            "second-stage champion gate (default: 0.53)."
        ),
    )
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Save lightweight policy-only candidates every N updates even "
            "when the champion gate rejects them; zero disables."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "After saving a rejected candidate, restore the accepted champion "
            "before the next PPO update."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Clear PPO and BC-replay optimizer moments whenever a rejected "
            "candidate is rolled back to the champion."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep decision-limit trajectories as terminal zero-return loss "
            "samples while still replacing them for exact valid-game quotas. "
            "The default preserves legacy exclusion behavior."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Opt in to treating invalid observations, battle-step errors, and "
            "decision-limit attempts as terminal outcome-zero losses. Only the "
            "final --failed-loss-tail-transitions learner rows are retained, "
            "and a replacement is still collected for the valid-game quota."
        ),
    )
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "Maximum final learner transitions kept from each failed attempt "
            "when --failed-attempt-as-loss is enabled (default: 32)."
        ),
    )
    ...
    if args.candidate_snapshot_interval < 0:
        raise ValueError("--candidate-snapshot-interval must be >= 0")
    if args.rollback_on_gate_failure and args.champion_gate_interval <= 0:
        raise ValueError(
            "--rollback-on-gate-failure requires --champion-gate-interval"
        )
    if (
        ...
        raise ValueError(
            "--reset-optimizer-on-gate-rollback requires "
            "--rollback-on-gate-failure"
        )
    ...
    opponent_meta_weights: dict[str, float] = {}
    for name, raw_weight in args.opponent_meta_weight:
        if name in opponent_meta_weights:
            raise ValueError(f"Duplicate --opponent-meta-weight name: {name}")
        try:
            weight = float(raw_weight)
        except ValueError as error:
            raise ValueError(
                f"Invalid --opponent-meta-weight value for {name!r}: "
                f"{raw_weight!r}"
            ) from error
        if not math.isfinite(weight) or weight <= 0.0:
            raise ValueError(
                f"--opponent-meta-weight for {name!r} must be finite and "
                "positive"
            )
        opponent_meta_weights[name] = weight
    opponent_base_quotas: dict[str, int] = {}
    ...
    if (
        args.champion_gate_interval < 0
        or args.champion_gate_games < 1
        or args.champion_gate_games % 2 != 0
        or args.champion_gate_confirmation_games < 1
        or args.champion_gate_confirmation_games % 2 != 0
        or args.champion_gate_initial_wait_updates < 0
        or args.champion_gate_failure_cooldown_updates < 0
        or args.champion_gate_success_cooldown_updates < 0
        or args.champion_pool_gate_games < 0
        or args.champion_pool_gate_games % 2 != 0
        or not math.isfinite(args.champion_pool_gate_min_win_rate)
        or not 0.0 <= args.champion_pool_gate_min_win_rate <= 1.0
        or not math.isfinite(args.champion_gate_min_win_rate)
        or not 0.0 <= args.champion_gate_min_win_rate <= 1.0
    ):
        raise ValueError(
            "Champion gate requires a non-negative interval, a positive even "
            "game count, a positive even confirmation game count, and a "
            "finite win-rate threshold in [0, 1]"
        )
    if args.champion_gate_soft_min_win_rate is not None and (
        not math.isfinite(args.champion_gate_soft_min_win_rate)
        or not 0.0 <= args.champion_gate_soft_min_win_rate
        <= args.champion_gate_min_win_rate
    ):
        raise ValueError(
            "--champion-gate-soft-min-win-rate must be finite and in "
            "[0, --champion-gate-min-win-rate]"
        )
    if args.champion_gate_interval > 0 and (
        args.actor_reduction_audit_only
        or args.actor_value_gradient_audit_only
    ):
        raise ValueError(
            "Champion gate is incompatible with gradient audit-only modes"
        )
    if args.max_game_decisions < 1:
        raise ValueError("--max-game-decisions must be positive")
    validate_failed_attempt_loss_configuration(
        args.failed_attempt_as_loss,
        args.failed_loss_tail_transitions,
        args.truncation_as_loss,
    )
    ...
    if opponent_meta_weights:
        if set(opponent_meta_weights) != permanent_opponent_names:
            raise ValueError(
                "--opponent-meta-weight names must exactly cover every "
                "permanent opponent"
            )
        if args.opponent_win_rate_window_games < 1:
            raise ValueError(
                "--opponent-win-rate-window-games must be positive"
            )
        inverse_factors = (
            args.opponent_inverse_min_factor,
            args.opponent_inverse_max_factor,
        )
        if (
            any(
                not math.isfinite(value) or value <= 0.0
                for value in inverse_factors
            )
            or args.opponent_inverse_min_factor
            > args.opponent_inverse_max_factor
        ):
            raise ValueError("Invalid opponent inverse factor bounds")
        if args.opponent_quota_mode == "legacy" and opponent_weights:
            raise ValueError(
                "Legacy rolling --opponent-meta-weight is mutually "
                "exclusive with static --opponent-weight"
            )
    ...
    if args.opponent_quota_mode == "legacy" and quota_arguments_present:
        raise ValueError(
            ...
            "fixed, adaptive, or meta_inverse_window"
        )
    if args.opponent_quota_mode != "legacy":
        ...
        if args.opponent_quota_mode == "meta_inverse_window":
            if set(opponent_meta_weights) != permanent_opponent_names:
                raise ValueError(
                    "meta_inverse_window --opponent-meta-weight names must "
                    "exactly cover every permanent opponent"
                )
            if opponent_audit:
                raise ValueError(
                    "meta_inverse_window does not use --opponent-audit"
                )
        elif opponent_meta_weights:
            raise ValueError(
                "--opponent-meta-weight requires "
                "--opponent-quota-mode meta_inverse_window"
            )
    if (
        ...
        raise ValueError(
            "--opponent-quota-seat-balance requires an exact opponent "
            "quota mode"
        )
    if (
    ...
    if args.ppo_objective == "standard":
        ...
        if set(opponent_base_quotas) != permanent_opponent_names:
            raise ValueError(
        ...
        missing_rollout_groups = {
            name
            for name in permanent_opponent_names
        ...
        if args.constrained_gradient_mode != "scalar":
            ...
            if permanent_opponent_names != core_names:
                raise ValueError(
    ...
    config = PPOConfig(
        ...
        opponent_win_rate_window_games=(
            args.opponent_win_rate_window_games
        ),
        opponent_inverse_min_factor=args.opponent_inverse_min_factor,
        opponent_inverse_max_factor=args.opponent_inverse_max_factor,
        ...
        truncation_as_loss=args.truncation_as_loss,
        failed_attempt_as_loss=args.failed_attempt_as_loss,
        failed_loss_tail_transitions=(
            args.failed_loss_tail_transitions
        ),
        champion_gate_interval=args.champion_gate_interval,
        champion_gate_games=args.champion_gate_games,
        champion_gate_min_win_rate=(
            args.champion_gate_min_win_rate
        ),
        champion_gate_threshold_inclusive=(
            args.champion_gate_threshold_inclusive
        ),
        champion_gate_soft_min_win_rate=(
            args.champion_gate_soft_min_win_rate
        ),
        champion_gate_confirmation_games=(
            args.champion_gate_confirmation_games
        ),
        champion_gate_initial_wait_updates=(
            args.champion_gate_initial_wait_updates
        ),
        champion_gate_failure_cooldown_updates=(
            args.champion_gate_failure_cooldown_updates
        ),
        champion_gate_success_cooldown_updates=(
            args.champion_gate_success_cooldown_updates
        ),
        champion_pool_gate_games=args.champion_pool_gate_games,
        champion_pool_gate_min_win_rate=(
            args.champion_pool_gate_min_win_rate
        ),
        candidate_snapshot_interval=args.candidate_snapshot_interval,
        rollback_on_gate_failure=args.rollback_on_gate_failure,
    ...
    resume_sampling_reweight_state: dict[str, Any] | None = None
    ...
    if args.resume:
        ...
        saved_sampling_reweight_state = resume.get(
            "opponent_sampling_reweight_state"
        )
        if saved_sampling_reweight_state is not None:
            if not isinstance(saved_sampling_reweight_state, dict):
                raise ValueError(
                    "Resume checkpoint opponent sampling reweight state is "
                    "malformed"
                )
            resume_sampling_reweight_state = saved_sampling_reweight_state
    ...
    # Decks change the official engine state but do not change a frozen
    # policy's parameters.  The recent-meta league binds many decks to the
    # same BC checkpoint, so keep one immutable CUDA model per checkpoint
    # content hash instead of deep-copying the same model for every deck.
    # Apart from saving memory this also lets rollout inference combine those
    # opponents into one large batch by model identity.
    frozen_model_cache: dict[str, dict[str, Any]] = {
        file_sha256(Path(config.bc_checkpoint).resolve()): {
            "model": opponents[0].model,
            "feature_version": bc_checkpoint.get("feature_version"),
            "model_config": model_config,
            "expected_deck_hash": checkpoint_single_deck_hash(bc_checkpoint),
            "source": str(Path(config.bc_checkpoint).resolve()),
        }
    }
    ...
    for spec in config.extra_opponents:
        ...
        cache_entry = frozen_model_cache.get(checkpoint_digest)
        reused_frozen_model = cache_entry is not None
        if cache_entry is None:
            ...
            if extra_model_config != model_config:
                mismatches = {
                    ...
                    for key in sorted(
                        model_config.keys() | extra_model_config.keys()
                    )
            ...
            frozen = frozen_copy(
                ...
                "",
                permanent=True,
                canonical_order=(extra_feature_version == BC_FEATURE_VERSION),
            )
            cache_entry = {
                "model": frozen.model,
                "feature_version": extra_feature_version,
                "model_config": extra_model_config,
                "expected_deck_hash": checkpoint_single_deck_hash(
                    extra_checkpoint
                ),
                "source": str(checkpoint_path),
            }
            frozen_model_cache[checkpoint_digest] = cache_entry
            del extra_model, extra_checkpoint
        extra_feature_version = str(cache_entry["feature_version"])
        extra_deck = read_deck(deck_path)
        extra_deck_hash = compute_deck_hash(extra_deck)
        expected_deck_hash = cache_entry["expected_deck_hash"]
        if (
            expected_deck_hash is not None
            and extra_deck_hash != expected_deck_hash
        ):
            raise ValueError(
                f"Deck hash mismatch for {checkpoint_path}: {deck_path} has "
                f"{extra_deck_hash}, checkpoint config expects "
                f"{expected_deck_hash}"
            )
        opponents.append(
            FrozenOpponent(
                name=spec["name"],
                model=cache_entry["model"],
                deck=list(extra_deck),
        ...
        extra_run_info.append(
            {
                **spec,
                ...
                "shared_frozen_model": reused_frozen_model,
                "shared_model_source": cache_entry["source"],
            }
        )
    if args.resume:
    ...
    permanent_opponent_names = [
        opponent.name for opponent in opponents if opponent.permanent
    ]
    opponent_sampling_reweighter = (
        OpponentSamplingReweighter(
            opponent_names=permanent_opponent_names,
            meta_weights=config.opponent_meta_weights,
            window_games=config.opponent_win_rate_window_games,
            inverse_min_factor=config.opponent_inverse_min_factor,
            inverse_max_factor=config.opponent_inverse_max_factor,
        )
        if (
            config.opponent_quota_mode == "legacy"
            and config.opponent_meta_weights
        )
        else None
    )
    if opponent_sampling_reweighter is not None:
        if (
            resume_sampling_reweight_state is not None
            and not config.reset_opponent_quota_on_resume
        ):
            opponent_sampling_reweighter.load_state_dict(
                resume_sampling_reweight_state
            )
        elif (
            resume is not None
            and resume_sampling_reweight_state is None
            and not config.reset_optimizer_on_resume
        ):
            raise ValueError(
                "Starting rolling opponent reweighting from a checkpoint "
                "without its state requires --reset-optimizer-on-resume"
            )
    elif (
        resume_sampling_reweight_state is not None
        and not config.reset_optimizer_on_resume
        ...
        raise ValueError(
            "Disabling rolling opponent reweighting on resume requires "
            "--reset-optimizer-on-resume"
        )
    ...
    resume_metadata = {
        ...
        "opponent_sampling_reweight_state": (
            "resume_checkpoint"
            if resume_sampling_reweight_state is not None
            else "absent"
        ),
    ...
    run_info = {
        ...
        "opponent_sampling_reweight_initial_state": (
            opponent_sampling_reweighter.state_dict()
            if opponent_sampling_reweighter is not None
            else None
        ),
        "opponent_sampling_reweight_resume": {
            "checkpoint_state_present": (
                resume_sampling_reweight_state is not None
            ),
            "state_loaded": (
                opponent_sampling_reweighter.resume_state_loaded
                if opponent_sampling_reweighter is not None
                else False
            ),
            "state_reset": (
                opponent_sampling_reweighter is not None
                and resume_sampling_reweight_state is not None
                and config.reset_opponent_quota_on_resume
            ),
        },
    ...
    champion_model: EntityOptionPolicy | None = None
    champion_update: int | None = None
    if config.champion_gate_interval > 0 and existing_best:
        champion_model = instantiate_model_from_checkpoint(
            existing_best,
            bc_checkpoint,
            device,
        )
        ...
        for parameter in champion_model.parameters():
            parameter.requires_grad_(False)
        champion_update = int(existing_best.get("update", start_update - 1))
    ...
    if should_run_initial_evaluation(
        ...
        if (
            ...
            and (
                config.champion_gate_interval == 0
                or champion_model is None
            )
        ):
            ...
            save_ppo_checkpoint(
                ...
                opponent_sampling_reweighter=opponent_sampling_reweighter,
                objective_state=objective_state,
            )

    if config.champion_gate_interval > 0 and champion_model is None:
        champion_model = copy.deepcopy(current_model).to(device)
        champion_model.eval()
        for parameter in champion_model.parameters():
            parameter.requires_grad_(False)
        champion_update = start_update - 1
        champion_initial_metrics: dict[str, Any] = {
            "evaluation": initial_eval,
            "champion_gate": {
                "status": "initial_champion",
                "champion_update": champion_update,
                "games": config.champion_gate_games,
                "minimum_win_rate": config.champion_gate_min_win_rate,
            },
        }
        if config.eval_all_permanent_opponents:
            champion_initial_metrics.update(
                {
                    "evaluation_by_opponent": (
                        initial_evaluation_by_opponent
                    ),
    ...
    champion_gate_initial_wait = (
        config.champion_gate_initial_wait_updates
        or config.champion_gate_interval
    )
    champion_gate_failure_cooldown = (
        config.champion_gate_failure_cooldown_updates
        or config.champion_gate_interval
    )
    champion_gate_success_cooldown = (
        config.champion_gate_success_cooldown_updates
        or config.champion_gate_interval
    )
    champion_next_gate_update: int | None = (
        start_update - 1 + champion_gate_initial_wait
        if config.champion_gate_interval > 0
        else None
    )
    if resume is not None and config.champion_gate_interval > 0:
        saved_gate_schedule = (resume.get("metrics") or {}).get(
            "champion_gate_schedule"
        )
        saved_next_gate = (
            saved_gate_schedule.get("next_eligible_update")
            if isinstance(saved_gate_schedule, dict)
            else None
        )
        if saved_next_gate is not None:
            saved_next_gate = int(saved_next_gate)
            if saved_next_gate < start_update:
                raise ValueError(
                    "Resume champion gate schedule predates the first "
                    f"continued update: next={saved_next_gate} "
                    f"start={start_update}"
                )
            champion_next_gate_update = saved_next_gate
            log(
                "champion_gate_schedule_resumed="
                + json.dumps(
                    {
                        "checkpoint_update": resume_update,
                        "next_eligible_update": champion_next_gate_update,
                    },
                    sort_keys=True,
                )
            )
    ...
    with metrics_path.open(metrics_mode, encoding="utf-8") as metrics_file:
        for update in range(start_update, config.updates + 1):
            ...
            if (
                config.champion_gate_interval > 0
                and champion_next_gate_update is not None
                and update >= champion_next_gate_update
            ):
                if champion_model is None or champion_update is None:
                    raise RuntimeError("Champion gate was not initialized")
                previous_champion_update = champion_update
                challenge = evaluate_head_to_head(
                    current_model,
                    champion_model,
                    deck,
                    model_config,
                    ...
                    min(config.environments, config.champion_gate_games),
                    config.max_game_decisions,
                    current_canonical_order=False,
                ...
                promotion_path = "not_promoted"
                if config.champion_gate_soft_min_win_rate is None:
                    champion_promotion = champion_gate_promotes(
                        challenge,
                        config.champion_gate_min_win_rate,
                        inclusive=(
                            config.champion_gate_threshold_inclusive
                        ),
                    )
                    if champion_promotion:
                        promotion_path = (
                            "inclusive_hard"
                            if config.champion_gate_threshold_inclusive
                            else "strict_hard"
                        )
                elif champion_gate_promotes(
                    challenge,
                    config.champion_gate_min_win_rate,
                    inclusive=True,
                ):
                    champion_promotion = True
                    promotion_path = "direct_hard"
                elif champion_gate_promotes(
                    challenge,
                    config.champion_gate_soft_min_win_rate,
                    inclusive=True,
                ):
                    confirmation_challenge = evaluate_head_to_head(
                        current_model,
                        champion_model,
                        deck,
                        model_config,
                        device,
                        config.champion_gate_confirmation_games,
                        min(
                            config.environments,
                            config.champion_gate_confirmation_games,
                        ),
                    ...
                    challenge = combine_head_to_head_evaluations(
                        [initial_challenge, confirmation_challenge]
                    )
                    champion_promotion = champion_gate_promotes(
                        challenge,
                        config.champion_gate_soft_min_win_rate,
                    ...
                    promotion_path = (
                        "confirmed_soft"
                        if champion_promotion
                        else "soft_confirmation_failed"
                    )
                else:
                    champion_promotion = False
                mirror_gate_passed = champion_promotion
                opponent_pool_gate = None
                opponent_pool_gate_passed = (
                    True if config.champion_pool_gate_games == 0 else False
                )
                if mirror_gate_passed and config.champion_pool_gate_games > 0:
                    ...
                    if pool_sampling_weights is None:
                        pool_sampling_weights = [1.0] * len(opponents)
                    opponent_pool_gate = evaluate_weighted_opponent_pool(
                        current_model,
                        opponents,
                        pool_sampling_weights,
                        deck,
                        model_config,
                        device,
                        config.champion_pool_gate_games,
                        config.environments,
                        config.max_game_decisions,
                    )
                    opponent_pool_gate_passed = champion_gate_promotes(
                        opponent_pool_gate,
                        config.champion_pool_gate_min_win_rate,
                        inclusive=False,
                    )
                    champion_promotion = opponent_pool_gate_passed
                    promotion_path = (
                        promotion_path + "+pool_passed"
                        if opponent_pool_gate_passed
                        else promotion_path + "+pool_failed"
                    )
                champion_gate = {
                    **challenge,
                    ...
                    "strict_threshold": (
                        config.champion_gate_soft_min_win_rate is None
                        and not config.champion_gate_threshold_inclusive
                    ),
                    "inclusive_threshold": (
                        config.champion_gate_soft_min_win_rate is not None
                        or config.champion_gate_threshold_inclusive
                    ),
                    "initial_challenge": initial_challenge,
                    "confirmation_challenge": confirmation_challenge,
                    "promotion_path": promotion_path,
                    "mirror_gate_passed": mirror_gate_passed,
                    "opponent_pool_gate": opponent_pool_gate,
                    "opponent_pool_gate_games": (
                        config.champion_pool_gate_games
                    ),
                    "opponent_pool_gate_min_win_rate": (
                        config.champion_pool_gate_min_win_rate
                    ),
                    "opponent_pool_gate_passed": opponent_pool_gate_passed,
                    "promoted": champion_promotion,
                }
                if champion_promotion:
                    champion_model = copy.deepcopy(current_model).to(device)
                    champion_model.eval()
                ...
                cooldown_updates = (
                    champion_gate_success_cooldown
                    if champion_promotion
                    else champion_gate_failure_cooldown
                )
                champion_next_gate_update = update + max(cooldown_updates, 1)
                champion_gate.update(
                    {
                        "cooldown_reason": (
                            "promotion" if champion_promotion else "rejection"
                        ),
                        "cooldown_updates": int(cooldown_updates),
                        "next_eligible_update": champion_next_gate_update,
                    }
                )
            result = {
                ...
                "champion_gate_schedule": {
                    "next_eligible_update": champion_next_gate_update,
                    "remaining_updates": (
                        max(champion_next_gate_update - update, 0)
                        if champion_next_gate_update is not None
                        else None
                    ),
                    "initial_wait_updates": champion_gate_initial_wait,
                    "failure_cooldown_updates": (
                        champion_gate_failure_cooldown
                    ),
                    "success_cooldown_updates": (
                        champion_gate_success_cooldown
                    ),
                },
            }
            candidate_snapshot_due = (
                config.candidate_snapshot_interval > 0
                and update % config.candidate_snapshot_interval == 0
            )
            if candidate_snapshot_due or champion_gate is not None:
                candidate_path = (
                    output_dir / "candidates" / f"update-{update:04d}.pt"
                )
            ...
            if (
                ...
                and not champion_promotion
                and config.rollback_on_gate_failure
            ):
                if champion_model is None:
                    raise RuntimeError("Cannot roll back without a champion model")
                rollback = rollback_to_champion(
                    current_model,
                    champion_model,
                    optimizer,
                    replay_optimizer,
                ...
                rollback.update(
                    {
                        "restored_champion_update": champion_update,
                        "candidate_snapshot": result.get("candidate_snapshot"),
                    }
                )
                champion_gate["rollback"] = rollback
            if config.eval_all_permanent_opponents:
                result.update(
                    {
            ...
            if champion_gate:
                message += (
                    f" champion_win={champion_gate['win_rate']:.4f} "
                    f"champion_W/L/D={champion_gate['wins']}/"
                    f"{champion_gate['losses']}/"
                    f"{champion_gate['draws']} "
                    f"promoted={champion_gate['promoted']}"
                )
            log(message)
            ...
            elif config.champion_gate_interval > 0 and champion_promotion:
                best_eval = evaluation
                best_evaluation_by_opponent = evaluation_by_opponent
                if evaluation_selection_score is not None:
                    best_selection_score = evaluation_selection_score
                save_ppo_checkpoint(
                    output_dir / "best.pt",
                    current_model,
            ...
            emit_league_snapshot = (
                champion_promotion
                if config.champion_gate_interval > 0
                else update % config.snapshot_interval == 0
            )
            if emit_league_snapshot:
    ...
    summary = {
        ...
        "champion_gate": {
            "enabled": config.champion_gate_interval > 0,
            "interval": config.champion_gate_interval,
            "initial_wait_updates": champion_gate_initial_wait,
            "failure_cooldown_updates": champion_gate_failure_cooldown,
            "success_cooldown_updates": champion_gate_success_cooldown,
            "next_eligible_update": champion_next_gate_update,
            "games": config.champion_gate_games,
            "minimum_win_rate": config.champion_gate_min_win_rate,
            "opponent_pool_games": config.champion_pool_gate_games,
            "opponent_pool_minimum_win_rate": (
                config.champion_pool_gate_min_win_rate
            ),
            "threshold_inclusive": (
                config.champion_gate_threshold_inclusive
            ...
            "confirmation_games": config.champion_gate_confirmation_games,
            "champion_update": champion_update,
        },
        ...
        "opponent_sampling_reweight_final_state": (
            opponent_sampling_reweighter.state_dict()
            if opponent_sampling_reweighter is not None
            else None
        ),
