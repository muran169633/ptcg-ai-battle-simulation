#!/usr/bin/env python3
"""Apply a frozen Gold-policy BC stage after the current U456 PPO policy.
...
def main() -> None:
    ...
    common: dict[str, Any] = {
        "schema_version": "ptcg-u456-post-special-bc-v1",
    ...
    if set(replay_steps_after) != {expected_final_step}:
        raise RuntimeError("Replay AdamW step did not advance exactly 16 times")
    if (
        repair_audit.nested_sha256(parent.get("optimizer_state_dict"))
        != ppo_optimizer_hash_before
    ):
    ...
    output_checkpoint = output_dir / "special-bc-0016.pt"
