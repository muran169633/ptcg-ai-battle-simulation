#!/usr/bin/env python3
"""Build evaluation-only submitted-U472 plus scaled U476 actor-delta endpoints."""
...
U476 = ROOT / "artifacts/design202608168_submittedu472_ppo4x192_u476/B_gold_league/seed-202608168/checkpoints/update-0476.pt"
OUTPUT_ROOT = ROOT / "artifacts/design202608170_parent_plus_u476_actor_delta"
INPUT_SHA256 = {
    PARENT: "c0436d54fff5e4da62d94c3002775c9e4b9ee848c5c59dd4422943cd71ad7018",
    U476: "29e2f4eb2fad789a985724c9fe4ef64840b5ceee9c6d7097676bbb45a9c6ca6e",
}
ACTOR10 = (
...
def main() -> int:
    ...
    checkpoints = {
        ...
        for label, path in (("parent", PARENT), ("u476", U476))
    }
    parent = checkpoints["parent"]["model_state_dict"]
    u476 = checkpoints["u476"]["model_state_dict"]
    if set(parent) != set(u476):
        raise RuntimeError("model-state keys differ")
    if changed(parent, u476) != sorted((*ACTOR10, *VALUE4)):
        raise RuntimeError("U476 did not change exactly head14")
    for state in (parent, u476):
    ...
    manifest: dict[str, Any] = {
        "schema_version": "ptcg-design202608170-parent-plus-u476-actor-delta-v1",
        "created_at_utc": created_at,
        "formula": "actor10 only: W_parent + alpha * (W_U476 - W_parent); value4 and all other tensors remain bit-exact parent",
    ...
    for endpoint, alpha in ENDPOINTS:
        state = {name: tensor.detach().cpu().clone() for name, tensor in parent.items()}
        for name in ACTOR10:
            value = (
                parent[name].detach().cpu().double()
                + alpha * (u476[name].detach().cpu().double() - parent[name].detach().cpu().double())
            ).to(dtype=parent[name].dtype)
            if not bool(torch.isfinite(value).all()):
                raise RuntimeError(f"non-finite endpoint tensor: {name}")
            state[name] = value
        if changed(parent, state) != sorted(ACTOR10):
            raise RuntimeError(f"{endpoint} changed unexpected tensors")
        model_hash = state_sha(state)
        slim = {key: copy.deepcopy(checkpoints["parent"][key]) for key in REQUIRED_SLIM_KEYS}
        slim.update({
            ...
            "ppo_actor_delta_backoff": {
                "schema_version": "ptcg-design202608170-parent-plus-u476-actor-delta-endpoint-v1",
                "created_at_utc": created_at,
                "endpoint": endpoint,
                "alpha": alpha,
                "mutable_parameter_names": list(ACTOR10),
        ...
        filename = f"parent-plus-u476-actor-{endpoint}.pt"
        ...
        manifest["endpoints"].append({
            ...
            "model_state_sha256": model_hash,
            "changed_parameter_names": list(ACTOR10),
        })
    publish(
