#!/usr/bin/env python3
"""Hash-bound in-process launcher for the frozen CW23 specialist adapter."""
...
LAUNCHER = TOOLS / "launch_cw23_payload_specialist60_gate_v1.py"
ADAPTER = TOOLS / "run_cw23_payload_specialist60_gate_v1.py"
ADAPTER_SHA256 = "b0949812caee8105e59b9eaa0b9b97f10781649ac97357b9c6b53e570ad2625c"
FROZEN_MODE = 0o555
...
def open_adapter_held() -> tuple[bytes, dict[str, Any], int, tuple[int, ...]]:
    ...
    if not stat.S_ISREG(before.st_mode) or stat.S_ISLNK(before.st_mode) or before.st_nlink != 1 or stat.S_IMODE(before.st_mode) != FROZEN_MODE:
    ...
    try:
        ...
        if not identity(before) == identity(opened) == locked_identity == identity(after_path):
            raise LaunchError("adapter: identity changed during held-FD read")
        payload = b"".join(chunks)
        digest = sha256_bytes(payload)
        if len(payload) != after_fd.st_size or digest != ADAPTER_SHA256:
...
def main() -> None:
    ...
    checks = {
        ...
        "argv_exact": sys.argv in ([str(LAUNCHER)], [str(LAUNCHER), "--audit-only"]),
        "targets_absent": not lexists(ATTEMPT) and not lexists(OUTPUT),
    }
    if not all(checks.values()):
        raise LaunchError(f"launcher runtime/target drift: {checks}")
    launcher_source, launcher_record = read_regular(LAUNCHER, None, FROZEN_MODE, "launcher")
    adapter_source, adapter_record, adapter_fd, adapter_identity = open_adapter_held()
    binding = {
        "schema_version": "ptcg-cw23-specialist60-hash-bound-launch-v1",
    ...
    code = compile(adapter_source, str(ADAPTER), "exec", dont_inherit=True, optimize=0)
    ...
    pending: tuple[BaseException, Any] | None = None
    post_error: BaseException | None = None
    try:
        exec(code, namespace, namespace)
    except BaseException as error:
        pending = (error, error.__traceback__)
    finally:
        try:
            if identity(os.fstat(adapter_fd)) != adapter_identity or identity(ADAPTER.lstat()) != adapter_identity:
            ...
            os.close(adapter_fd)
    try:
        _, adapter_after = read_regular(ADAPTER, ADAPTER_SHA256, FROZEN_MODE, "adapter after execution")
        _, launcher_after = read_regular(LAUNCHER, launcher_record["sha256"], FROZEN_MODE, "launcher after execution")
    ...
    if post_error is not None:
        if pending is not None:
            raise LaunchError(f"adapter raised and post-execution identity audit also failed: {post_error}") from pending[0]
        raise post_error
    if pending is not None:
        raise pending[0].with_traceback(pending[1])
