#!/usr/bin/env python3
"""Run the frozen duration-32 PPO recipe with only cosine LR scheduling.

The duration-32 constant-LR experiment is the control. This launcher preserves
its model inputs, rollout budget, opponent quotas, optimizer settings, replay
settings, seed, and endpoint-only checkpoint selection. The only modeling
change is ``--learning-rate-schedule constant`` to ``cosine``.
"""
...
OUTPUT_DIR = (
    ROOT
    / "artifacts"
    / "ppo_gold_duration32_cosine_tailconsensus5_ow8_ctx34q2_ordered_seed20260739_20260729"
    / "B_gold_league"
    / "seed-20260739"
)
...
def main() -> None:
    ...
    set_flag(command, "--updates", "472")
    set_flag(command, "--eval-interval", "472")
    set_flag(command, "--checkpoint-interval", "472")
    set_flag(command, "--seed", "20260739")
    ...
    if command[command.index("--schedule-start-update") + 1] != "441":
        raise RuntimeError("Cosine phase must span updates 441 through 472")
