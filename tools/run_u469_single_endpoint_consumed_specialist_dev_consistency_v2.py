...
authorize broad, Gold, packaging, upload, or submission.  It replaces a
rejected-before-run v1 whose CUDA preflight preceded its atomic attempt marker;
the frozen v1 files remain provenance only and its marker/output must stay
absent forever.

The executor validates one hash-bound preregistration, requires all targets
absent, performs a CPU-only pre-execution rehash, atomically consumes an
attempt marker containing ``cuda_preflight_pending``, and only then may touch
CUDA.  It attempts all six panels exactly once in their declared order,
requires all 60 gates, and rehashes every immutable input both before and after
execution.  It performs no training or network access.
"""
...
TOOLS = ROOT / "tools"
SCRIPT = TOOLS / "run_u469_single_endpoint_consumed_specialist_dev_consistency_v2.py"
BUILDER = TOOLS / "build_u469_consumed_specialist_dev_consistency_preregistration_v2.py"
...
PREREGISTRATION_SCHEMA = (
    "ptcg-u469-single-endpoint-consumed-specialist-dev-consistency-preregistration-v2"
)
MANIFEST_SCHEMA = (
    "ptcg-u469-single-endpoint-consumed-specialist-dev-consistency-manifest-v2"
)
DECISION_SCHEMA = (
    "ptcg-u469-single-endpoint-consumed-specialist-dev-consistency-decision-v2"
)
ATTEMPT_SCHEMA = (
    "ptcg-u469-single-endpoint-consumed-specialist-dev-consistency-attempt-v2"
)
PREREGISTRATION_STATUS = "locked_before_specialist_dev_consistency"

ARTIFACT_ID = "u469_single_endpoint_consumed_specialist_dev_consistency_20260803_v2"
...
PREDECESSOR_V1_RUNNER = TOOLS / (
    "run_u469_single_endpoint_consumed_specialist_dev_consistency_v1.py"
)
PREDECESSOR_V1_RUNNER_SHA256 = (
    "83768c1fe6fce5252b95c015e3be97e4acb0ede07f4248b2f4143c5013bcc47d"
)
PREDECESSOR_V1_BUILDER = TOOLS / (
    "build_u469_consumed_specialist_dev_consistency_preregistration_v1.py"
)
PREDECESSOR_V1_BUILDER_SHA256 = (
    "706ed66dcd73a2214223a6ed676069fc072b2c4409d08b88972b890bef8dc36a"
)
PREDECESSOR_V1_PREREGISTRATION = ROOT / "artifacts" / (
    "u469_single_endpoint_consumed_specialist_dev_consistency_20260802_v1."
    "preregistration.json"
)
PREDECESSOR_V1_PREREGISTRATION_SHA256 = (
    "e61e711b1b32ea814c7b6db79694468789de848559659a1535fb3bd8ae7278b0"
)
PREDECESSOR_V1_ATTEMPT_MARKER = ROOT / "artifacts" / (
    ".ptcg-u469_single_endpoint_consumed_specialist_dev_consistency_20260802_v1-"
    "specialist-dev-consistency-attempt.json"
)
PREDECESSOR_V1_OUTPUT_ROOT = ROOT / "artifacts" / (
    "u469_single_endpoint_consumed_specialist_dev_consistency_20260802_v1."
    "specialist_dev_consistency"
)
PREDECESSOR_REJECTED_BEFORE_RUN_REASON = (
    "v1_cuda_preflight_preceded_atomic_attempt_marker_so_cuda_could_be_touched_"
    "without_consuming_the_one_shot"
)
...
def expected_predecessor_v1_binding() -> dict[str, Any]:
    return {
        "runner": {
            "path": root_relative_text(PREDECESSOR_V1_RUNNER),
            "sha256": PREDECESSOR_V1_RUNNER_SHA256,
        },
        "builder": {
            "path": root_relative_text(PREDECESSOR_V1_BUILDER),
            "sha256": PREDECESSOR_V1_BUILDER_SHA256,
        },
        "preregistration": {
            "path": root_relative_text(PREDECESSOR_V1_PREREGISTRATION),
            "sha256": PREDECESSOR_V1_PREREGISTRATION_SHA256,
        },
        "attempt_marker": {
            "path": root_relative_text(PREDECESSOR_V1_ATTEMPT_MARKER),
            "required_state": "absent_forever",
            "absent_at_v2_lock": True,
        },
        "output_root": {
            "path": root_relative_text(PREDECESSOR_V1_OUTPUT_ROOT),
            "required_state": "absent_forever",
            "absent_at_v2_lock": True,
        },
        "predecessor_rejected_before_run_reason": (
...
def predecessor_v1_absence_evidence(
    phase: str,
    *,
    raise_on_failure: bool,
) -> dict[str, Any]:
    checks = {
        "attempt_marker_absent_by_lstat": absent_by_lstat(
            PREDECESSOR_V1_ATTEMPT_MARKER
        ),
        "output_root_absent_by_lstat": absent_by_lstat(PREDECESSOR_V1_OUTPUT_ROOT),
    }
    result = {
        "phase": phase,
        "pass": all(checks.values()),
        "checks": checks,
        "attempt_marker": root_relative_text(PREDECESSOR_V1_ATTEMPT_MARKER),
        "output_root": root_relative_text(PREDECESSOR_V1_OUTPUT_ROOT),
    }
    if raise_on_failure and not result["pass"]:
        raise ProtocolError(
            f"predecessor v1 targets must remain absent forever: {checks}"
        )
    return result


def verify_predecessor_v1(
    declared: Any,
) -> tuple[dict[str, Any], dict[str, Path]]:
    expected = expected_predecessor_v1_binding()
    if not isinstance(declared, Mapping) or dict(declared) != expected:
        raise ProtocolError("predecessor v1 binding drift")

    runner_payload, runner_evidence = read_frozen_source(
        PREDECESSOR_V1_RUNNER,
        PREDECESSOR_V1_RUNNER_SHA256,
        "rejected predecessor v1 runner",
    )
    _, builder_evidence = read_frozen_source(
        PREDECESSOR_V1_BUILDER,
        PREDECESSOR_V1_BUILDER_SHA256,
        "rejected predecessor v1 builder",
    )
    prereg_path, prereg_payload, prereg_evidence = legacy.verify_binding(
        declared["preregistration"],
        "rejected predecessor v1 preregistration",
        expected_path=PREDECESSOR_V1_PREREGISTRATION,
        expected_sha256=PREDECESSOR_V1_PREREGISTRATION_SHA256,
    )
    if prereg_evidence["mode_octal"] != "0444":
        raise ProtocolError("rejected predecessor v1 preregistration must stay frozen")
    predecessor_prereg = legacy.strict_json_loads(
        prereg_payload,
        "rejected predecessor v1 preregistration",
    )
    try:
        predecessor_runner_source = runner_payload.decode("utf-8")
    except UnicodeDecodeError as error:
        raise ProtocolError("rejected predecessor v1 runner is not UTF-8") from error
    content_checks = {
        "runner_binding_exact": predecessor_prereg.get("executor", {}).get("path")
        == root_relative_text(PREDECESSOR_V1_RUNNER)
        and predecessor_prereg.get("executor", {}).get("sha256")
        == PREDECESSOR_V1_RUNNER_SHA256,
        "builder_binding_exact": predecessor_prereg.get("generator")
        == declared["builder"],
        "attempt_marker_path_exact": predecessor_prereg.get("output_rule", {}).get(
            "attempt_marker"
        )
        == root_relative_text(PREDECESSOR_V1_ATTEMPT_MARKER),
        "output_root_path_exact": predecessor_prereg.get("output_rule", {}).get("root")
        == root_relative_text(PREDECESSOR_V1_OUTPUT_ROOT),
        "rejected_order_observed": 0
        <= predecessor_runner_source.find("cuda = legacy.cuda_preflight()")
        < predecessor_runner_source.find("legacy.atomic_create(ATTEMPT_MARKER"),
        "rejected_reason_exact": declared.get(
            "predecessor_rejected_before_run_reason"
        )
        == PREDECESSOR_REJECTED_BEFORE_RUN_REASON,
    }
    if not all(content_checks.values()):
        raise ProtocolError(f"rejected predecessor v1 content drift: {content_checks}")
    absence = predecessor_v1_absence_evidence(
        "predecessor_v1_validation",
    ...
    return {
        ...
        "runner": PREDECESSOR_V1_RUNNER,
        "builder": PREDECESSOR_V1_BUILDER,
...
def validate_preregistration(
    ...
    predecessor_evidence, predecessor_paths = verify_predecessor_v1(
        prereg.get("predecessor_v1")
    )
    ...
    expected_output_rule = {
        ...
        "attempt_created_before_cuda_preflight": True,
        "attempt_marker_cuda_field_at_creation": "cuda_preflight_pending",
        "predecessor_v1_targets_required_absent": True,
    ...
    immutable_bindings.extend(
        {
            "label": f"rejected predecessor v1 {name}",
            "path": predecessor_paths[name],
            "sha256": expected_predecessor_v1_binding()[name]["sha256"],
        }
        for name in ("runner", "builder", "preregistration")
    )
    if len(immutable_bindings) != 30:
        raise ProtocolError("immutable input binding count must be exactly 30")
    ...
    return prereg, {
        ...
        "predecessor_v1": predecessor_evidence,
...
def static_audit() -> dict[str, Any]:
    ...
    predecessor, _ = verify_predecessor_v1(expected_predecessor_v1_binding())
    ...
    checks = {
        ...
        "predecessor_v1_attempt_marker_absent_by_lstat": absent_by_lstat(
            PREDECESSOR_V1_ATTEMPT_MARKER
        ),
        "predecessor_v1_output_root_absent_by_lstat": absent_by_lstat(
            PREDECESSOR_V1_OUTPUT_ROOT
        ),
    }
    if not all(checks.values()):
        raise ProtocolError(f"U469 dev executor static audit failed: {checks}")
    return {
        "schema_version": "ptcg-u469-consumed-specialist-dev-consistency-static-v2",
...
def run(preregistration_path: Path, expected_sha256: str) -> dict[str, Any]:
    ...
    predecessor_absence_before_rehash = predecessor_v1_absence_evidence(
        "before_cpu_pre_execution_rehash",
        raise_on_failure=True,
    )
    pre_execution_rehash = legacy.rehash_bound_inputs(
        evidence["immutable_bindings"],
        phase="cpu_pre_execution_before_attempt",
        raise_on_failure=True,
    )
    predecessor_absence_before_attempt = predecessor_v1_absence_evidence(
        "after_cpu_pre_execution_rehash_before_attempt",
        raise_on_failure=True,
    )
    marker_payload = canonical_json_bytes(
        {
            ...
            "predecessor_v1": expected_predecessor_v1_binding(),
            "predecessor_v1_absence": {
                "before_cpu_pre_execution_rehash": (
                    predecessor_absence_before_rehash
                ),
                "after_cpu_pre_execution_rehash_before_attempt": (
                    predecessor_absence_before_attempt
                ),
            },
            "classification": CLASSIFICATION,
            "authorization": AUTHORIZATION,
            "pre_execution_rehash": pre_execution_rehash,
            "cuda_preflight": "cuda_preflight_pending",
        }
    )
    legacy.atomic_create(ATTEMPT_MARKER, marker_payload, mode=0o444)
    marker_observed_payload, marker_evidence = legacy.read_plain_file(
        ATTEMPT_MARKER,
        "v2 atomic attempt marker before CUDA preflight",
    )
    if (
        marker_observed_payload != marker_payload
        or marker_evidence["sha256"] != sha256_bytes(marker_payload)
        or marker_evidence["mode_octal"] != "0444"
    ):
        raise ProtocolError("v2 atomic attempt marker verification failed before CUDA")
    ...
    predecessor_absence_post_execution = predecessor_v1_absence_evidence(
        "post_execution",
        raise_on_failure=False,
    )
    ...
    passed = (
        ...
        and bool(predecessor_absence_post_execution["pass"])
        and len(gates) == 60
    ...
    manifest = {
        ...
        "predecessor_v1": evidence["predecessor_v1"],
        "predecessor_v1_absence": {
            "before_cpu_pre_execution_rehash": predecessor_absence_before_rehash,
            "after_cpu_pre_execution_rehash_before_attempt": (
                predecessor_absence_before_attempt
            ),
            "post_execution": predecessor_absence_post_execution,
        },
