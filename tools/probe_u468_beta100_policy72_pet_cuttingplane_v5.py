#!/usr/bin/env python3
"""Thin frozen extension of combined v1 over every policy-affecting parameter.

The mutable vector is the frozen actor6 prefix followed by embedding, input
encoders, positions, all four Transformer blocks, and final LayerNorm: seventy-two
FP32 tensors / 5,111,297 scalars.  Count and value heads remain frozen because
their gradients are identically zero for the initial policy/NLL closure.  Only
the actor6 prefix is projected out of the frozen P/E/T span.
...
TOOLS = ROOT / "tools"
SCRIPT = TOOLS / "probe_u468_beta100_policy72_pet_cuttingplane_v5.py"
EXPECTED_PYTHON = Path("/home/xxc/miniconda3/envs/my_project_env/bin/python")
SCHEMA = "ptcg-u468-beta100-policy72-pet-cuttingplane-v5"
...
EMBEDDING_ELEMENTS = 4194304
FRONTEND_ELEMENTS = 57856
TRANSFORMER_ELEMENTS = 793344
SHARED_POLICY_ELEMENTS = EMBEDDING_ELEMENTS + FRONTEND_ELEMENTS + TRANSFORMER_ELEMENTS
NORM_ELEMENTS = SHARED_POLICY_ELEMENTS
...
INPUT_SHARED_NAMES = (
    ...
    "global_encoder.1.weight",
    "global_encoder.1.bias",
    "entity_encoder.0.weight",
    "entity_encoder.0.bias",
    "entity_encoder.1.weight",
    "entity_encoder.1.bias",
    "option_encoder.0.weight",
    "option_encoder.0.bias",
    "option_encoder.1.weight",
    "option_encoder.1.bias",
    "kind_embedding.weight",
    "state_position.weight",
    "option_position.weight",
)
TRANSFORMER_PARAMETER_SUFFIXES = (
...
TRANSFORMER_BLOCK_NAMES = tuple(
    f"transformer.layers.{layer}.{suffix}"
    for layer in range(4)
    for suffix in TRANSFORMER_PARAMETER_SUFFIXES
)
FINAL_NORM_NAMES = ("transformer.norm.weight", "transformer.norm.bias")
SHARED_POLICY_NAMES = INPUT_SHARED_NAMES + TRANSFORMER_BLOCK_NAMES + FINAL_NORM_NAMES
EXPECTED_BETA_VARIABLE72_SHA256 = (
    "f59749203321899d647115d78e517431af4d36c0e1a9c1d179bb315872171d45"
)
EXPECTED_BETA_OTHER8_SHA256 = (
    "118f49121f4049f30b998f6bc62e34a4f8345113ddf5d4c05e0afe1de22151aa"
)
...
NORM_NAMES = SHARED_POLICY_NAMES
...
def validate_runtime() -> None:
    ...
    if len(VARIABLE_NAMES) != 72 or COMBINED_ELEMENTS != 5111297:
        raise RuntimeError("v5 variable manifest drift")
    if HARD_COMBINED_L2 != BASE.HARD_COMBINED_L2:
        raise RuntimeError("v5 attempted to relax the combined radius")


def endpoint_audit_v5(vector: np.ndarray, q: np.ndarray) -> dict[str, Any]:
    value = np.asarray(vector, dtype=np.float64)
    if value.shape != (COMBINED_ELEMENTS,):
        raise RuntimeError(f"v5 endpoint shape drift: {value.shape}")
    if q.shape != (COMBINED_ELEMENTS, 3):
        raise RuntimeError(f"v5 embedded-Q shape drift: {q.shape}")
    components = q.T @ value
    actor = value[:ACTOR_ELEMENTS]
    embedding_start = ACTOR_ELEMENTS
    frontend_start = embedding_start + EMBEDDING_ELEMENTS
    transformer_start = frontend_start + FRONTEND_ELEMENTS
    embedding = value[embedding_start:frontend_start]
    frontend = value[frontend_start:transformer_start]
    transformer = value[transformer_start:]
    combined_l2 = float(np.linalg.norm(value))
    actor_l2 = float(np.linalg.norm(actor))
    embedding_l2 = float(np.linalg.norm(embedding))
    frontend_l2 = float(np.linalg.norm(frontend))
    transformer_l2 = float(np.linalg.norm(transformer))
    ...
    return {
        ...
        "frontend_l2": frontend_l2,
        "transformer_l2": transformer_l2,
        "all_shared_policy_l2": float(np.linalg.norm(value[ACTOR_ELEMENTS:])),
...
def build_combined_geometry_v5(basis: Mapping[str, Any]) -> dict[str, Any]:
    q_actor = np.ascontiguousarray(basis["q"].numpy(), dtype=np.float64)
    if q_actor.shape != (ACTOR_ELEMENTS, 3):
        raise RuntimeError("v5 actor PET Q shape drift")
    ...
    if residual > 1.0e-12 or bool(np.any(q[ACTOR_ELEMENTS:] != 0.0)):
        raise RuntimeError("v5 embedded PET geometry drift")
    ...
    checks = {
        "variable_tensor_count_exact_72": len(variable) == 72,
        "combined_elements_exact_5111297": (
        ...
        "other_tensor_count_exact_8": len(other) == 8,
        "variable72_sha_exact": (
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v5 beta100 state geometry drift: {checks}")
    return {
        "q": q,
        "q_actor": q_actor,
        "report": {
            ...
            "transformer_elements": TRANSFORMER_ELEMENTS,
            "shared_policy_elements": SHARED_POLICY_ELEMENTS,
            ...
            "pet_span": "frozen actor6 P/E/T with 5045504 zero shared-policy rows",
            "beta_variable72_sha256": EXPECTED_BETA_VARIABLE72_SHA256,
            "beta_other8_sha256": EXPECTED_BETA_OTHER8_SHA256,
            "checks": checks,
        },
    }
...
def _binding_values() -> dict[str, Any]:
    return {
        ...
        "COMBINED_ELEMENTS": COMBINED_ELEMENTS,
        "EXPECTED_BETA_VARIABLE8_SHA256": EXPECTED_BETA_VARIABLE72_SHA256,
        "EXPECTED_BETA_OTHER72_SHA256": EXPECTED_BETA_OTHER8_SHA256,
        "build_combined_geometry": build_combined_geometry_v5,
        "endpoint_audit": endpoint_audit_v5,
    }
...
def static_audit() -> dict[str, Any]:
    ...
    checks = {
        ...
        "scope_exact_72_tensors_5111297": (
            len(VARIABLE_NAMES) == 72 and COMBINED_ELEMENTS == 5111297
        ),
        "shared_partition_exact_4194304_plus_57856_plus_793344": (
            EMBEDDING_ELEMENTS == 4194304
            and FRONTEND_ELEMENTS == 57856
            and TRANSFORMER_ELEMENTS == 793344
            and SHARED_POLICY_ELEMENTS == 5045504
            and NORM_ELEMENTS == SHARED_POLICY_ELEMENTS
        ),
        "one_hard_combined_radius_unchanged": HARD_COMBINED_L2 == 1.0e-3,
        "solver_self_test_under_v5_bindings": solver_test["pass"],
    ...
    if not all(checks.values()):
        raise RuntimeError(f"v5 static audit failed: {checks}")
    return {
        "status": "static_policy72_v5_audit_passed",
        "checks": checks,
...
def _normalize_result_labels(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(scope, dict):
        scope["other_state_tensors_exact"] = 8
        scope["embedding_elements"] = EMBEDDING_ELEMENTS
        scope["frontend_elements"] = FRONTEND_ELEMENTS
        scope["transformer_elements"] = TRANSFORMER_ELEMENTS
        scope["shared_policy_elements"] = SHARED_POLICY_ELEMENTS
    integrity = result.get("final_integrity")
    if isinstance(integrity, dict):
        if "variable8_tensor_restored_exact" in integrity:
            integrity["variable72_tensor_restored_exact"] = integrity.pop(
                "variable8_tensor_restored_exact"
            )
        if "other72_sha_after" in integrity:
            integrity["other8_sha_after"] = integrity.pop("other72_sha_after")
        if "other72_restored_exact" in integrity:
            integrity["other8_restored_exact"] = integrity.pop(
...
def run_probe(
    ...
    def enriched_consumer(payload_source: Mapping[str, Any]) -> None:
        if candidate_consumer is None:
            raise RuntimeError("internal v5 consumer contract drift")
        ...
        if int(vector.numel()) != COMBINED_ELEMENTS:
            raise RuntimeError("v5 consumer vector shape drift")
        embedding_start = ACTOR_ELEMENTS
        frontend_start = embedding_start + EMBEDDING_ELEMENTS
        transformer_start = frontend_start + FRONTEND_ELEMENTS
        payload["actual_embedding_delta_l2"] = float(
            vector[embedding_start:frontend_start].double().norm()
        )
        payload["actual_frontend_delta_l2"] = float(
            vector[frontend_start:transformer_start].double().norm()
        )
        payload["actual_transformer_delta_l2"] = float(
            vector[transformer_start:].double().norm()
        )
        payload["actual_shared_policy_delta_l2"] = float(
            vector[ACTOR_ELEMENTS:].double().norm()
        )
        payload["v5_variable_names"] = list(VARIABLE_NAMES)
...
def cache_audit() -> dict[str, Any]:
    ...
    result["status"] = "policy72_v5_cache_audit_passed_zero_write_train_only"
    result["self"] = read_regular_bytes(SCRIPT, None, "policy72 v5 self")[1]
...
def main() -> None:
    ...
    if args.mode == "static-audit":
        result: dict[str, Any] = {
            ...
            "self": read_regular_bytes(SCRIPT, None, "policy72 v5 self")[1],
