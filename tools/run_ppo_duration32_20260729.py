#!/usr/bin/env python3
"""Execute the frozen 32-update extension of the ordered-replay PPO recipe.

This launcher has no packaging, upload, network, or submission path.  It loads
the previously frozen duration-8 command and changes only the terminal update
budget, terminal intervals, output directory, and random seed.
"""
...
OUTPUT_DIR = (
    ROOT
    / "artifacts"
    / "ppo_gold_duration32_tailconsensus5_ow8_ctx34q2_ordered_seed20260739_20260729"
    / "B_gold_league"
    / "seed-20260739"
)
...
def main() -> None:
    ...
    if "--resume-learner-weights" not in command:
        raise RuntimeError("Missing frozen resume learner-weight mode")
