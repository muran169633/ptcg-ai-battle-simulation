#!/usr/bin/env python3
"""CW12 consumed-valid official-B256 cutting-plane probe.
...
SCRIPT = TOOLS / "probe_u468_cw11_consumed_valid_official6_cw12_cuttingplane_v1.py"
SCHEMA = "ptcg-cw12-consumed-valid-official-b256-cuttingplane-v1"
SEED = 202608207
FROZEN_MODE = 0o555
...
def static_source_audit(source: bytes) -> dict[str, Any]:
    ...
    checks = {
        ...
        "official_matmul_precision_high_scoped_and_restored": all(
            token in segments.get("run_cw12_consumer", "")
            for token in (
...
def frozen_modules() -> dict[str, ModuleType]:
    return {
        ...
        "cw10": import_frozen(
            CW10, MODULE_SHAS[CW10], EXPECTED_INPUT_MODES[CW10], "cw12_frozen_cw10"
        ),
        "cw9": import_frozen(
            CW9, MODULE_SHAS[CW9], EXPECTED_INPUT_MODES[CW9], "cw12_frozen_cw9"
        ),
        ...
        "geometry": import_frozen(
            ...
            "cw12_frozen_geometry",
        ),
        "ram": import_frozen(
            RAM, MODULE_SHAS[RAM], EXPECTED_INPUT_MODES[RAM], "cw12_frozen_ram"
        ),
        "legacy": import_frozen(
            FORENSIC_ANALYZER,
            MODULE_SHAS[FORENSIC_ANALYZER],
            EXPECTED_INPUT_MODES[FORENSIC_ANALYZER],
            "cw12_frozen_forensic_analyzer",
        ),
    }
...
def full_stream_separation_oracle(
    ...
    *,
    ...
    hard = {
        ...
        observed_transition_occurrence_identities
        == expected_transition_occurrence_identities,
        "favorable_transitions_retained": all(
            value["retained"]
            for value in transition_records
            if value["classification"] == "favorable"
        ),
        "new_harm_cut_cardinality_exact": len(new_cuts) == len(new_harms),
        "new_harm_cuts_unique": len({cut_identity(value) for value in new_cuts})
        == len(new_cuts),
    }
    if not all(hard.values()):
    ...
    return {
        ...
        "new_harms": new_harms,
        "deterministic_new_cuts": new_cuts,
    ...
    *,
    ...
    checks = {
        ...
        "forensic_favorable_retention_pass": bool(
            oracle["hard_checks"]["favorable_transitions_retained"]
        ),
...
def run_outer_cutting_plane(
    ...
    *,
    ...
    if not required_context.issubset(context):
        raise RuntimeError("CW12 incoming CW11 consumer schema drift")
    incoming_checks = {
    ...
    if not all(initial_checks.values()):
        raise RuntimeError(f"CW12 initial-state audit failed: {initial_checks}")
    ...
    if (
        ...
        raise RuntimeError("CW12 initial 34+4 active cut ledger drift")
    ...
    if initial_active_gates["violated_count"] != 4:
        raise RuntimeError("CW12 initial fixed repair violation count drift")
    ...
    return {
        ...
        "downstream_contract": {
            ...
            "broad_requires_new_CW12_hash_bound_one_shot_runner": True,
        },
    }


def run_cw12_consumer(
    context: Mapping[str, Any],
    modules: Mapping[str, ModuleType],
    *,
    user_candidate_consumer: Any = None,
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError(
                "CW12 local finally failed to restore CW11 or matmul precision"
            )
...
def run_probe(
    ...
    *,
    ...
    def internal_consumer(context: Mapping[str, Any]) -> None:
        if holder:
            raise RuntimeError("CW12 live consumer called more than once")
        holder.update(
            run_cw12_consumer(
                context,
                modules,
    ...
    if not holder:
        raise RuntimeError("CW12 consumer was not reached by exact CW11 reconstruction")
    final_checks = {
    ...
    if not all(final_checks.values()):
        raise RuntimeError(f"CW12 outer reconstruction/finally failed: {final_checks}")
    return {
...
def static_result() -> dict[str, Any]:
    ...
    if stat.S_IMODE(SCRIPT.stat().st_mode) != FROZEN_MODE:
        raise RuntimeError("CW12 script must be frozen mode 0555")
    ...
    return {
        "schema_version": SCHEMA,
        "status": "static_ready_run_implemented",
        "classification": dict(CLASSIFICATION),
        "contract": {
...
def main() -> None:
    ...
    if args.mode == "static":
        ...
        cw11 = import_frozen(
            ...
            MODULE_SHAS[CW11_PROBE],
            EXPECTED_INPUT_MODES[CW11_PROBE],
            "cw12_main_frozen_cw11",
        )
        source, self_evidence = cw11.read_regular_bytes(
            SCRIPT,
            None,
            "CW12 consumed-valid solver",
            expected_mode=FROZEN_MODE,
        )
        source_audit = static_source_audit(source)
        if not source_audit["pass"]:
            raise RuntimeError(f"CW12 source audit failed: {source_audit}")
        primary_source, primary_evidence = cw11.read_regular_bytes(
            PRIMARY,
            MODULE_SHAS[PRIMARY],
            "CW12 frozen primary source",
            expected_mode=FROZEN_MODE,
        )
        primary = import_frozen(
            PRIMARY,
