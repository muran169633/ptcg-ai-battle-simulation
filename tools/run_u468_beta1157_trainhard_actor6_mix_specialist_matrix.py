#!/usr/bin/env python3
"""Evaluate the frozen beta1.157 train-hard actor6 mixed-retention P1/P2 sweep.

This launcher deliberately does not train.  It consumes the completed,
hash-bound two-endpoint training manifest, evaluates both published endpoints
on the same six specialist/retention panels, and only then selects the smallest
fully eligible step.  Evaluation uses ``tools/evaluate_policy_bc.py`` through
the PPO ``model_forward`` path (CUDA bfloat16 autocast) with policy-greedy
ordering.

Audit-only execution validates every immutable input, command, endpoint model
state, absolute gate, and absent output target without writing.  Formal
execution is one-shot: it preclaims all 12 outputs, runs all 12 commands even
when an earlier child fails, writes a terminal manifest and decision, and seals
the directory.  This tool contains no broad, Gold, packaging, network, upload,
or submission implementation.
"""
...
PREREGISTRATION_SCHEMA = (
    "ptcg-u468-beta1157-trainhard-actor6-mix-specialist-"
    "preregistration-v1"
)
...
EXPECTED_STEPS = (1, 2)
EXPECTED_ENDPOINT_NAMES = ("p001", "p002")
EXPECTED_BRANCH = (
    "ppo_u468_beta1157_trainhard_actor6_mix102_77_77_p1p2_design202608101"
)
EXPECTED_TRAINING_MANIFEST_SHA256 = (
    "51b971ffdea581207ea68f6a85b11af9cc5234338b4026b2c7bf33f4eba70c94"
)
EXPECTED_TRAINING_COMPLETION_SEAL_SHA256 = (
    "612139bb3bc363ccdfcc9233acade576f8badce74180dd5e62c1d16c8864ba54"
)
EXPECTED_EVALUATION_COUNT = 12
EXPECTED_PREREGISTRATION_STATUS = "locked_before_single_zero_write_audit"
EXPECTED_PANELS = (
...
def validate_protocol(
    ...
    if protocol.get("schema_version") != PREREGISTRATION_SCHEMA or (
        protocol.get("status") != EXPECTED_PREREGISTRATION_STATUS
    ):
    ...
    if training_file["sha256"] != args.expected_training_manifest_sha256 or (
        training_file["sha256"] != EXPECTED_TRAINING_MANIFEST_SHA256
    ):
        raise ValueError("Training manifest SHA-256 differs from the frozen binding")
    training_manifest = strict_json_loads(training_raw, "training manifest")
    if training_manifest.get("branch") != EXPECTED_BRANCH:
        raise ValueError("Training manifest branch differs from the frozen branch")
    ...
    training_seal_path, training_seal_file = file_binding(
        training_seal_binding,
        "training completion seal",
    )
    if training_seal_file["sha256"] != EXPECTED_TRAINING_COMPLETION_SEAL_SHA256:
        raise ValueError("Training completion seal differs from the frozen binding")
    ...
    if not isinstance(endpoint_bindings, list) or len(endpoint_bindings) != 2:
        raise ValueError("Exactly two endpoint bindings are required")
    ...
    if observed_endpoint_identity != list(zip(EXPECTED_ENDPOINT_NAMES, EXPECTED_STEPS)):
        raise ValueError("Endpoint identity/order must be P1/P2")
    ...
    if len(set(endpoint_paths.values())) != 2 or len({
        ...
        raise ValueError("P1/P2 endpoint paths, hashes, and pointers must be unique")
    ...
    if output_rule != {
        ...
        "evaluation_count_exact": EXPECTED_EVALUATION_COUNT,
    ...
    if not isinstance(evaluations, list) or len(evaluations) != len(expected_matrix):
        raise ValueError("Ordered evaluation matrix must contain exactly 12 entries")
    ...
    if len(set(outputs)) != EXPECTED_EVALUATION_COUNT or protocol.get("ordered_command_matrix_sha256") != (
    ...
    *,
    ...
    try:
        ...
        marker_payload = canonical_json_file_bytes({
            "schema_version": "ptcg-u468-beta1157-trainhard-actor6-mix-specialist-attempt-v1",
        ...
        if len(results) != EXPECTED_EVALUATION_COUNT:
            raise RuntimeError("Internal error: formal loop did not produce 12 records")
        ...
        manifest = {
            "schema_version": "ptcg-u468-beta1157-trainhard-actor6-mix-specialist-manifest-v1",
            ...
            "cuda_preflight": cuda,
            "evaluation_count_required": EXPECTED_EVALUATION_COUNT,
            ...
            endpoint_decisions = [
                {
                    ...
                    "step": step,
                    "eligible": False,
                    "reason": "one_or_more_of_12_evaluations_failed",
                }
                for endpoint, step in zip(EXPECTED_ENDPOINT_NAMES, EXPECTED_STEPS)
            ]
        ...
        decision = {
            "schema_version": "ptcg-u468-beta1157-trainhard-actor6-mix-specialist-decision-v1",
            ...
            "all_12_outputs_completed_before_decision": all_completed,
        ...
        completion = {
            "schema_version": "ptcg-u468-beta1157-trainhard-actor6-mix-specialist-completion-v1",
            "status": (
                "completed"
                if all_completed
            ...
            "evaluations_attempted": EXPECTED_EVALUATION_COUNT,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.mode == "audit-only":
        if args.formal_authorization is not None or (
            ...
            raise ValueError("Audit-only mode must not receive formal authorization")
        for target in (paths["marker"], paths["output_root"]):
            if target.exists() or target.is_symlink():
                raise FileExistsError(f"Audit-only target appeared: {target}")
        print(json.dumps({
            "schema_version": "ptcg-u468-beta1157-trainhard-actor6-mix-specialist-audit-v1",
            "status": "audit_only_passed_zero_writes",
            "preregistration_sha256": preregistration_sha256,
            "training_manifest_sha256": protocol["training_manifest"]["sha256"],
            "training_completion_seal_sha256": protocol["training_completion_seal"]["sha256"],
            "launcher_sha256": protocol["launcher"]["sha256"],
            "ordered_command_matrix_sha256": protocol["ordered_command_matrix_sha256"],
            "evaluation_count": EXPECTED_EVALUATION_COUNT,
            "cuda_preflight": cuda,
            "writes": 0,
